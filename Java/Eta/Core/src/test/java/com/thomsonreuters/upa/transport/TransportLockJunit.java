...
public class TransportLockJunit
{
     ...
     * </ul>
     */
    class UpajServer implements Runnable
    {
        boolean DEBUG = false;
        ...
        /**
         * A blocking UpajServer is not supported for junits.
         * 
         * @param bindOptions
         * @param acceptOptions
         * @param globalLocking
         */
        public UpajServer(BindOptions bindOptions, AcceptOptions acceptOptions,
                TestArgs args)
        {
        ...
        {
            if (DEBUG)
                System.out.println("UpajServer: terminate() entered.");
        ...
        {
            if (DEBUG)
                System.out.println("UpajServer: setupServer() entered");

            if (_bindOptions.channelsBlocking())
            {
                _errorMsg = "Blocking UpajServer is not supported in junits.";
                return false;
            }

            if (DEBUG)
                System.out.println("UpajServer: setupServer() binding");
            ...
            if (DEBUG)
                System.out.println("UpajServer: setupServer() opening selector");
            ...
            if (DEBUG)
                System.out.println("UpajServer: setupServer() completed successfully");
        ...
        {
            if (DEBUG)
                System.out.println("UpajServer: closeSockets() entered");
        ...
        {
            if (DEBUG)
                System.out.println("UpajServer: initializeChannel() entered");
            ...
            {
                if (DEBUG)
                    System.out.println("UpajServer: initializeChannel() SUCCESS - verifying compressionType is "
                            + _bindOptions.compressionType());
        ...
        private void processStatistics(TransportBuffer msgBuf, ReadArgs readArgs)
        {
            ...
            if (DEBUG)
                System.out.println("UpajServer: processStatistics() processed message byte="
        ...
        {
            if (DEBUG)
                System.out.println("UpajServer: processRead() entered");
            ...
            do
            {
                ...
                if (!_running)
                {
                    ...
                    {
                        System.out.println("UpajServer: processRead() channel.read() is returning more to read, but runtime has expired.");
                        break;
                    }
                    else
                    {
                        System.out.println("UpajServer: processRead() runtime expired, _messageCount="
                                + _messageCount);
                    }
        ...
        {
            if (DEBUG)
                System.out.println("UpajServer: processSelector() entered");
                ...
                if (_selector.select(TIMEOUTMS) > 0)
                {
                    ...
                    if (keySet != null)
                    {
                        ...
                        while (iter.hasNext())
                        {
                            ...
                            {
                                if (DEBUG)
                                    System.out.println("UpajServer: processSelector() accepting a connection");
                            ...
                            if (key.isReadable())
                            {
                                ...
                                if (DEBUG)
                                    System.out.println("UpajServer: processSelector() channel is readable, channelState="
                                            + channel.state());
        ...
        public void run()
        {
            ...
            if (DEBUG)
                System.out.println("UpajServer: run() entered");
            ...
            if (_errorMsg != null)
                System.out.println("UpajServer: run(): error occurred, " + _errorMsg);
            ...
            if (DEBUG)
                System.out.println("UpajServer: run() completed");
        }

    } // end of UpajServer class
     ...
     * </ul>
     */
    class UpajClient implements Runnable
    {
        boolean DEBUG = false;
         ...
         * @param globalLocking
         */
        public UpajClient(int id, int priority, Channel channel, TestArgs args)
        {
            assert (channel != null);
            ...
            if (DEBUG)
                System.out.println("UpajClient id=" + _id + ": created. priority=" + _priority);
        }
        ...
        {
            if (DEBUG)
                System.out.println("UpajClient id=" + _id + ": terminate() entered.");
        ...
        TransportBuffer getBuffer(int size, boolean packed)
        {
            ...
            while (msgBuf == null)
            {
                ...
                {
                    if (DEBUG)
                        System.out.println("UpajClient id=" + _id
        ...
        {
            if (DEBUG)
                System.out.println("UpajClient id=" + _id + ": writeMessage() entered");
            ...
            do
            {
                ...
                if (!_running)
                {
                    ...
                    else if (timeout < System.currentTimeMillis())
                    {
                        ...
                        System.out.println("UpajClient id=" + _id + " error=" + _errorMsg);
            ...
            if (retVal >= TransportReturnCodes.SUCCESS)
            {
                ...
                if (DEBUG)
                    System.out.println("UpajClient id=" + _id + ": writeMessage(): _messageCount="
                ...
                if ((_messageCount % _flushInterval) == 0)
                {
                    ...
                    if (retVal < TransportReturnCodes.SUCCESS)
                    {
                        _errorMsg = "writeMessage(): flush failed, retVal=" + retVal + " error="
                                + _error.text();
                        if (DEBUG)
                            System.out.println("UpajClient id=" + _id + ": writeMessage(): error="
                                    + _errorMsg);
                        return retVal;
                    }
                }
            ...
            if (DEBUG)
                System.out.println("UpajClient id=" + _id + ": writeMessage(): completed normally");
        ...
        private int writePackedMessage(int availableBytes, boolean callWriteWithPackedMessage)
        {
            ...
            if (callWriteWithPackedMessage)
            {
                ...
                for (int idx = 0; idx < _variablePackedMessageSize; idx++)
                {
                    if (msgBuf.data().position() == msgBuf.data().limit())
                        System.out.println("UpajClient.writePackedMessage(): position == limit!!!");
            ...
            else
            {
                _errorMsg = "writePackedMessage(): write failed, retVal=" + retVal + " error="
                        + _error.text();
                if (DEBUG)
                    System.out.println("UpajClient id=" + _id + ": writePackedMessage(): error="
                            + _errorMsg);
                return retVal;
            }
            if (DEBUG)
        ...
        public void run()
        {
            ...
            if (DEBUG)
                System.out.println("UpajClient id=" + _id + ": run() running");
            ...
            while (_running)
            {
                ...
                {
                    System.out.println("UpajClient id=" + _id + ": run(): _messageCount="
            ...
            if (_errorMsg != null)
                System.out.println("UpajClient id=" + _id + ": run(): error occurred, " + _errorMsg);

            if (Transport.uninitialize() != TransportReturnCodes.SUCCESS && _errorMsg == null)
            {
                _errorMsg = "UpajClient id=" + _id + ": Transport.uninitialize() failed.";
            }

            _runningState = RunningState.TERMINATED;
            // if (DEBUG)
            System.out.println("UpajClient id=" + _id + ": run() complete. messageCount="
                    + _messageCount);

        }
    } // end of UpajClient class
    ...
    /**
     * Create UpajClients for the specified number of threads for the specified
     * number of channels.
     * 
     * @param threadsPerChannels
     * @param channels created from caller.
     * @param upajClients populated with new clients
     * @param clientThreads populated with the new clients' threads.
     */
    public void startClientThreads(TestArgs args, Channel[] channels, UpajClient[] upajClients, Thread[] clientThreads)
    {
        int threadsPerChannels = args.threadsPerChannels;
        
        assert (channels != null);
        assert (upajClients != null);
        assert (clientThreads != null);

        int upajClientCount = -1;
        ...
        for (int channelIdx = 0; channelIdx < channels.length; channelIdx++)
        {
            ...
            {
                ++upajClientCount;
                upajClients[upajClientCount] = new UpajClient(upajClientCount, threadIdx,
                        channels[channelIdx], args);
                clientThreads[upajClientCount] = new Thread(upajClients[upajClientCount]);
                clientThreads[upajClientCount].start();
            }
        }
        return;
    }
     ...
     */
    public boolean waitForStateRunning(UpajServer server)
    {
        try
        {
            while (true)
    ...
    private void terminateServerAndClients(Thread serverThread, UpajServer server,
            Thread[] clientThreads, UpajClient[] upajClients, Channel[] channels)
    {
        System.out.println("terminateServerAndClients(): stopping clients");

        // instruct clients to stop
        for (UpajClient client : upajClients)
        {
            client.terminate();
        }
    ...
    private String compareStatistics(boolean packing, int clientMessageSize, UpajServer server,
            UpajClient[] upajClients)
    {
    ...
    public void testRunner(String testName, TestArgs args)
    {
        ...
        UpajClient[] upajClients = new UpajClient[args.clientSessionCount * args.threadsPerChannels];
        Thread[] clientThreads = new Thread[args.clientSessionCount * args.threadsPerChannels];

        // create the server thread and start it
        UpajServer server = new UpajServer(bindOptions, acceptOptions, args);
        ...
        else
        {
            ...
            startClientThreads(args, channels, upajClients, clientThreads);
            ...
            terminateServerAndClients(serverThread, server, clientThreads, upajClients, channels);
             ...
             */
            String result = compareStatistics(args.packing, args.messageSize, server, upajClients);
            if (result != null)
            {
