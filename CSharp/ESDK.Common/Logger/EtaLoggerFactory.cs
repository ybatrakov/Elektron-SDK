...
namespace ThomsonReuters.Common.Logger
{
    /// <summary>
    /// Creates ETA Logger
    /// </summary>
    public sealed class EtaLoggerFactory : ILoggerFactory
    {
        ...
        {
            var internalFactory = new LoggerFactory();
            ...
            if (LogToConsole)
                internalFactory.AddConsole(LogLevel);
            if (LogToMemory)
                internalFactory.AddMemory();
            if (LogToNLog)
                internalFactory.AddNLog();
            if (LogToDebug)
                internalFactory.AddDebug(LogLevel.Trace);
