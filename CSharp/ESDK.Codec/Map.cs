...
{
    /// <summary>
	/// The <seealso cref="Map"/> is a uniform container type of associated key - value pair
	/// entries, each known as a <seealso cref="MapEntry"/>. Each <seealso cref="MapEntry"/> contains an
	...
	/// </para>
	/// <para>
	/// The following sample illustrates the encoding of a <seealso cref="Map"/> containing
	/// <seealso cref="FieldList"/> values. The example encodes three <seealso cref="MapEntry"/> values
	...
	/// </para>
	/// <para>
	/// The following sample demonstrates the decoding of a <seealso cref="Map"/> and is
	/// structured to decode each entry to the contained value. This sample assumes
	/// that the housed container type is a <seealso cref="FieldList"/> and that the
	...
	/// uses the same <seealso cref="DecodeIterator"/> when calling the content's decoder
	...
	///             FieldList fieldList = new FieldList();
	///             retVal = fieldList.Decode(decIter, 0);
	///             // Continue decoding field entries. See <seealso cref="FieldList"/> for
	///             // decoding example.
	///         }
	///     }
	/// }
	...
	sealed public class Map : IXMLDecoder
    {
		...
		/// 3. Call Map.EncodeComplete()<para />
		/// </summary>
		...
		/// 1. Call Map.EncodeInit()<para />
		/// 2. Call MapEntry.Encode() or
		/// MapEntry.EncodeInit()..MapEntry.EncodeComplete() for each map entry in the list using the same buffer<para />
		/// 3. Call Map.EncodeComplete()<para />
		/// </summary>
		/// <param name="iter"> The encoder iterator. </param>
		...
		/// </summary>
		/// <seealso cref="Flags"/>
		/// <returns> true - if key is fid; false if not. </returns>
