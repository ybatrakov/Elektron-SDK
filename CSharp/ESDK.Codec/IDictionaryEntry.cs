/*|-----------------------------------------------------------------------------
 *|            This source code is provided under the Apache 2.0 license      --
 *|  and is provided AS IS with no warranty or guarantee of fit for purpose.  --
 *|                See the project's LICENSE.md for details.                  --
 *|           Copyright Thomson Reuters 2018. All rights reserved.            --
 *|-----------------------------------------------------------------------------
 */

using System;

namespace ThomsonReuters.Eta.Codec
{

	/// <summary>
	/// A data dictionary entry, containing field information and an enumeration table reference if present.
	/// </summary>
	/// <seealso cref="MfFieldTypes"/>
	/// <seealso cref="DataTypes"/>
	public interface IDictionaryEntry
	{
		/// <summary>
		/// Acronym.
		/// </summary>
		/// <returns> the acronym </returns>
		Buffer Acronym { get; }

		/// <summary>
		/// DDE Acronym.
		/// </summary>
		/// <returns> the ddeAcronym </returns>
		Buffer DdeAcronym { get; }

		/// <summary>
		/// The fieldId the entry corresponds to.
		/// </summary>
		/// <returns> the fid </returns>
		Int16 Fid { get; }

        /// <summary>
        /// The field to ripple data to.
        /// </summary>
        /// <returns> the rippleToField </returns>
        Int16 RippleToField { get; }

		/// <summary>
		/// Marketfeed Field Type.
		/// </summary>
		/// <returns> the fieldType </returns>
		SByte FieldType { get; }

		/// <summary>
		/// Marketfeed length.
		/// </summary>
		/// <returns> the length </returns>
		UInt16 Length { get; }

        /// <summary>
        /// Marketfeed enum length.
        /// </summary>
        /// <returns> the enumLength </returns>
        Byte EnumLength { get; }

		/// <summary>
		/// RWF type.
		/// </summary>
		/// <returns> the rwfType </returns>
		Byte RwfType { get; }

		/// <summary>
		/// RWF Length.
		/// </summary>
		/// <returns> the rwfLength </returns>
		UInt16 RwfLength { get; }

		/// <summary>
		/// A reference to the corresponding enumerated types table, if this field uses one.
		/// </summary>
		/// <returns> the enumTypeTable </returns>
		IEnumTypeTable EnumTypeTable { get; }
	}
}