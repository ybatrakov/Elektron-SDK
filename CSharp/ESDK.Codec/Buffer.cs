	...
	sealed public class Buffer
    {
        ...
        public CodecReturnCode Copy(ByteBuffer destBuffer)
        {
            ...
            if (_dataString is null)
            {
                ...
                destBuffer.Put(_data.Contents, _position, _position + length);
            }
            else
            {
                    ...
                    if ((destBuffer).DataString() is null)
                    {
                        ...
                        if (destByteBuffer != null)
                        {
                            if (length <= destBuffer.Length)
                            {
                                ...
                                {
                                    // ByteBuffer to ByteBuffer.
                                    destByteBuffer.Put(_data.Contents, _position, _position + length);
                                }
                                else
                                {
                                    // no need to check return code since we did range checking already.
        ...
        internal CodecReturnCode CopyWithOrWithoutByteBuffer(Buffer buffer)
        {
            ...
            {
                buffer._dataString = _dataString;
            }
            return Copy(buffer);
        }
		...
		///         ByteBuffer} then the <seealso cref="ByteBuffer"/>'s
		///         reference will be returned.</li>
		///         <li>If this Buffer is backed by a String, then a new
		///         <seealso cref="ByteBuffer"/> will be created from a new
        ...
        {
            if (!(bb is null) && bb.Position != 0) bb.Flip();
