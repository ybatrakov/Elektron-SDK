...
namespace ThomsonReuters.Eta.Codec
{
	...
	sealed public class DataDictionary
	{
        ...
        /// <summary>
		/// Creates <seealso cref="DataDictionary"/>.
		/// </summary>
		/// <returns> <seealso cref="DataDictionary"/> object
		/// </returns>
        ...
        /// <summary>
		/// Clears <seealso cref="DataDictionary"/>. This should be done prior to the first
		...
		/// </param>
		/// <returns> the dictionary entry if it exists, NULL otherwise </returns>
        ...
        /// </param>
        /// <returns> the enumerated type if it exists, NULL otherwise </returns>
        ...
        /// <seealso cref="DataDictionary"/> to load additional dictionaries (provided the
		...
		/// same <seealso cref="DataDictionary"/> to load additional dictionaries (provided
		...
		/// 
		/// Typical use:<para />
		/// 1. Call Msg.decode().<para />
		/// 2. If domainType is DICTIONARY, call this method.<para />
		...
		/// <param name="error"> ETA Error, to be populated in event of an error.
		/// </param>
		/// <returns> <seealso cref="CodecReturnCode"/>. If success, dictionary type is
		///         populated. If failure, dictionary type not available.
		/// </returns>
		/// <seealso cref="DecodeIterator"/>
		/// <seealso cref="ThomsonReuters.Eta.Rdm.Dictionary.Types"/>
        public CodecReturnCode ExtractDictionaryType(DecodeIterator iter, Int dictionaryType, out CodecError error)
		{
			...
			(tempBuffer).Data_internal(iter._buffer.Data(), iter._curBufPos, (iter._buffer.Length - iter._curBufPos));
		...
		///         <seealso cref="CodecReturnCode.DICT_PART_ENCODED"/> when encoding parts is
		///         success <seealso cref="CodecReturnCode.SUCCESS"/> for final part or single
		///         complete payload.
		/// </returns>
		/// <seealso cref="EncodeIterator"/>
		/// <seealso cref="ThomsonReuters.Eta.Rdm.Dictionary.VerbosityValues"/>
		...
		///            <seealso cref="ThomsonReuters.Eta.Rdm.Dictionary.VerbosityValues"/>. </param>
		/// <param name="error"> ETA error, to be populated in event of an error.
		/// </param>
		/// <returns> <seealso cref="CodecReturnCode"/>
		/// </returns>
		/// <seealso cref="DecodeIterator"/>
		/// <seealso cref="ThomsonReuters.Eta.Rdm.Dictionary.VerbosityValues"/>
		public CodecReturnCode DecodeFieldDictionary(DecodeIterator iter, int verbosity, CodecError error)
		{
            CodecReturnCode ret;
			int fid = 0;
			DictionaryEntryImpl newDictEntry;
		...
		public override string ToString()
		{
			...
			if (string.ReferenceEquals(dictionaryString, null))
			{
				...
				for (int i = 0; i <= MAX_FID - MIN_FID; i++)
				{
					...
					{
						sb.Append("  Fid=" + _entriesArray[i].Fid + " '" + _entriesArray[i].Acronym + "' '" + _entriesArray[i].DdeAcronym + "' Type=" + _entriesArray[i].FieldType + " RippleTo=" + _entriesArray[i].RippleToField + " Len=" + _entriesArray[i].Length + " EnumLen=" + _entriesArray[i].EnumLength + " RwfType=" + _entriesArray[i].RwfType + " RwfLen=" + _entriesArray[i].RwfLength + "\n");
					}
				}
        ...
        CodecReturnCode EncodeDataDictEntry(EncodeIterator iter, DictionaryEntryImpl entry, int verbosity, out CodecError error, LocalElementSetDefDb setDb)
        {
            ...
            elemEntry.DataType = DataTypes.ASCII_STRING;
            if ((ret = elemEntry.Encode(iter, entry.Acronym)) != CodecReturnCode.SUCCESS)
            {
                SetError(out error, "encodeElementEntry NAME '" + entry.Acronym.ToString() + "' failed " + ret);
                return CodecReturnCode.FAILURE;
            }
            ...
            {
                SetError(out error, "encodeElementEntry FID " + entry.Fid + " failed " + ret);
                return CodecReturnCode.FAILURE;
            }
            ...
            if ((ret = elemEntry.Encode(iter, tempInt)) != CodecReturnCode.SUCCESS)
            {
                SetError(out error, "encodeElementEntry RIPPLETO " + entry.RippleToField + " failed " + ret);
                return CodecReturnCode.FAILURE;
            }
            ...
            if ((ret = elemEntry.Encode(iter, tempInt)) != CodecReturnCode.SUCCESS)
            {
                SetError(out error, "encodeElementEntry TYPE " + entry.FieldType + " failed " + ret);
                return CodecReturnCode.FAILURE;
            }
            ...
            {
                SetError(out error, "encodeElementEntry LENGTH " + entry.Length + " failed " + ret);
                return CodecReturnCode.FAILURE;
            }
            ...
            if ((ret = elemEntry.Encode(iter, tempUInt)) != CodecReturnCode.SUCCESS)
            {
                SetError(out error, "encodeElementEntry RWFTYPE " + entry.RwfType + " failed " + ret);
                return CodecReturnCode.FAILURE;
            }
            ...
            {
                SetError(out error, "encodeElementEntry RWFLEN " + entry.RwfLength + " failed " + ret);
                return CodecReturnCode.FAILURE;
            }
            ...
            if (verbosity >= Dictionary.VerbosityValues.NORMAL)
            {
                ...
                if ((ret = elemEntry.Encode(iter, tempUInt)) != CodecReturnCode.SUCCESS)
                {
                    SetError(out error, "encodeElementEntry ENUMLENGTH " + entry.EnumLength + " failed " + ret);
                    return CodecReturnCode.FAILURE;
                }

                elemEntry.Name = ElementNames.FIELD_LONGNAME;
                elemEntry.DataType = DataTypes.ASCII_STRING;
                if ((ret = elemEntry.Encode(iter, entry.DdeAcronym)) != CodecReturnCode.SET_COMPLETE)
                {
                    SetError(out error, "encodeElementEntry LONGNAME Acronym '" + entry.DdeAcronym.ToString() + "' failed " + ret);
                    return CodecReturnCode.FAILURE;
                }
            }
