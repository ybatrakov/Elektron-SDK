...
namespace ThomsonReuters.Eta.Codec
{
	internal class DecodersToXML
	{
		...
		private static string XmlDumpElementListBegin(ElementList eList)
		{
			...
			xmlString.Append(Encodeindents());
			indents++;
			xmlString.Append("<elementList flags=\"0x").AppendFormat(string.Format("{0:X2}", eList.Flags));
		...
		private static string XmlDumpMsgBegin(IMsg msg, string tagName)
		{
			...
			xmlString.Append(" dataSize=\"").Append(msg.EncodedDataBody.Length).Append("\">\n");
		...
		private static string XmlDumpFieldListBegin(FieldList fList)
		{
			...
			xmlString.Append(Encodeindents());
			indents++;
			xmlString.Append("<fieldList flags=\"0x").AppendFormat(string.Format("{0:X2}", fList.Flags));
		...
		private static string XmlDumpMapEntryBegin(int keyPrimitiveType, MapEntry mapEntry, object mapKeyData)
		{
			...
			xmlString.Append("<mapEntry flags=\"0x").AppendFormat(string.Format("{0:X2}", mapEntry.Flags));
			...
			xmlString.Append(stringBuf.ToString()).Append("\" ");
		...
		private static string XmlDumpMapBegin(Map map)
		{
			...
			xmlString.Append(Encodeindents());
			indents++;
			xmlString.Append("<map flags=\"0x").AppendFormat(string.Format("{0:X2}", map.Flags));
		...
		private static string XmlDumpFilterItemBegin(FilterEntry filterItem)
		{
			...
			/* Don't print the data element for deleted rows, there should not be any. */
			xmlString.Append("<filterEntry id=\"").Append(filterItem.Id).Append("\" action=\"").Append(actionString).Append("\" flags=\"0x").AppendFormat(string.Format("{0:X2}", filterItem.Flags));
		...
		private static string XmlDumpFilterListBegin(FilterList fList)
		{
			...
			xmlString.Append("\" countHint=\"").Append(fList.TotalCountHint).Append("\" flags=\"0x").AppendFormat(string.Format("{0:X2}", fList.Flags));
		...
		private static string XmlDumpSeriesBegin(Series series)
		{
			...
			xmlString.Append("<series  flags=\"0x").AppendFormat(string.Format("{0:X2}", series.Flags));
			if (series.Flags != 0)
			{
				xmlString.Append(" (");
			}
		...
		private static string XmlDumpVectorEntryBegin(VectorEntry vectorEntry)
		{
			 ...
			 * any. */
			xmlString.Append("<vectorEntry index=\"").Append(vectorEntry.Index).Append("\" action=\"").Append(actionString).Append("\" flags=\"0x").AppendFormat(string.Format("{0:X2}", vectorEntry.Flags));
		...
		private static string XmlDumpVectorBegin(Vector vec)
		{
			...
			xmlString.Append(Encodeindents());
			indents++;
			xmlString.Append("<vector flags=\"0x").AppendFormat(string.Format("{0:X2}", vec.Flags));
