...
/* initialize lock */
RTR_C_ALWAYS_INLINE void seqMcastInitLock(rtr_mutex *seqMcastLock)
{
	rtr_mutex_error mterr = 0;

	RTR_MUTEX_INIT(*seqMcastLock, mterr);	
}

/* destroy lock */
RTR_C_ALWAYS_INLINE void seqMcastDestroyLock(rtr_mutex *seqMcastLock)
{
	rtr_mutex_error mterr = 0;

	RTR_MUTEX_DESTROY(*seqMcastLock, mterr);
}

/* get lock */
RTR_C_ALWAYS_INLINE void seqMcastGetLock(rtr_mutex *seqMcastLock)
{
	rtr_mutex_error mterr = 0;

	RTR_MUTEX_LOCK(*seqMcastLock, mterr);
}

/* release lock */
RTR_C_ALWAYS_INLINE void seqMcastUnlock(rtr_mutex *seqMcastLock)
{
	rtr_mutex_error mterr = 0;

	RTR_MUTEX_UNLOCK(*seqMcastLock, mterr);
}

/* try lock */
RTR_C_ALWAYS_INLINE RsslBool seqMcastTrylock(rtr_mutex *seqMcastLock)
{
	rtr_mutex_error mterr = 0;

	return RTR_MUTEX_TRYLOCK(*seqMcastLock, mterr);
}
...
RsslRet rsslSeqMcastCloseChannel(rsslChannelImpl* rsslChnlImpl, RsslError *error)
{
	...
	rsslBufImpl = (rsslBufferImpl *)rtr_qtool_first(&(rsslChnlImpl->activeBufferList));
	if (rsslBufImpl != NULL)
	{
...
/* Linux requires the socket to be bound to the remote address */
	...
	{
#if _WIN32
		if (ioctlsocket(socketId, FIONBIO, &arg ) == -1)
#else
...
RSSL_RSSL_SEQ_MCAST_IMPL_FAST(rsslBufferImpl*) rsslSeqMcastGetBuffer(rsslChannelImpl *rsslChnlImpl, RsslUInt32 size, RsslBool packedBuffer, RsslError *error)
{
	...
	pBufferImpl = (rsslBufferImpl*)rtr_qtool_takeqh(&(rsslChnlImpl->freeBufferList));
...
RSSL_RSSL_SEQ_MCAST_IMPL_FAST(RsslBuffer*) rsslSeqMcastRead(rsslChannelImpl* rsslChnlImpl, RsslReadOutArgs *readOutArgs, RsslRet *readRet, RsslError *error)
{
	...
	RsslInt32 hdrLen;
	RsslInt32 readFlags;
	RsslBool isRetransmit = RSSL_FALSE;

	if (!chnlLocking || chnlLocking && seqMcastTrylock(&pSeqMcastChannel->lock) ==  RSSL_TRUE)
	{
		if (pSeqMcastChannel->stillProcessingPacket == RSSL_FALSE) /* process a new packet from network */
		{
			...
			if ((cc = recvfrom(rsslChnlImpl->Channel.socketId, pSeqMcastChannel->inputBuffer.data, pSeqMcastChannel->maxMsgSize + SEQ_MCAST_MAX_HDR_LEN, 0, (struct sockaddr*)&srcAddr, &srcAddrLen)) < 0)
			{
			...
			iter = pSeqMcastChannel->inputBuffer.data;
		...
		else /* still processing previous packet */
		{
			...
			if(pSeqMcastChannel->processedPacketLen + pSeqMcastChannel->readBuffer.length > pSeqMcastChannel->inputBuffer.length)
			{
				...
				*readRet = RSSL_RET_FAILURE;
				_rsslSetError(error, NULL, RSSL_RET_FAILURE,  errno);
				snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> rsslRead() Error: 1007 Indicated message length is longer than remaining length of datagram.\n", __FILE__, __LINE__, pSeqMcastChannel->readBuffer.length);
				if (chnlLocking)
					seqMcastUnlock(&pSeqMcastChannel->lock);
				return NULL;
			}
