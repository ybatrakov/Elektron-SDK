...
************************************************************************/
int ripcValidServerName(char *name, int nlen)
{
	char *s;
	int  i;
...
************************************************************************/
int ripcGetServByName(char *serv_name)
{
	...
	else if ((!strcmp(serv_name, "rmds_rssl_sink")) || (!strcmp(serv_name, "rssl_consumer")))
		return host2net_u16(14002);
	else if ((!strcmp(serv_name, "rmds_rssl_source")) || (!strcmp(serv_name, "rssl_provider")))
		return host2net_u16(14003);
	else if ((!strcmp(serv_name,"triarch_sink"))  || (!strcmp(serv_name, "rmds_ssl_sink")))
		return host2net_u16(8101);
	else if ((!strcmp(serv_name,"triarch_src")) || (!strcmp(serv_name, "rmds_ssl_source")))
		return host2net_u16(8102);
	else if (!strcmp(serv_name,"triarch_dbms"))
		return host2net_u16(8103);
	else if (!strcmp(serv_name,"triarch_rrbp"))
...
int ripcBindSocket(u32 ipaddr, int portNum, RsslSocket fd)
{
...
int ipcConnected(RsslSocket fd)
{
	 ...
	 */
	retval=ripcReadyWrite(fd);
	if (retval == 1)
	{
		if (getpeername(fd,(struct sockaddr*)&baddr,&size) < 0)
		{
#ifdef _WIN32
	...
	else if (retval == 0)
	{
			 ...
			 */
		if ((retval = ripcReadyException(fd)) != 0)
		{
			/* if this is positive, we should try to read in order to get the errno */
			if (retval == 1)
			{
...
//static int ripcInitTrans(void *transport, RIPC_ERROR *error)
int ipcInitTrans(void *transport, ripcSessInProg *inPr, RsslError *error)
{
	return(1);
}

//int ripcRead( void *transport, char *buf, int max_len, ripcRWFlags flags, RIPC_ERROR *error)
int ipcRead( void *transport, char *buf, int max_len, ripcRWFlags flags, RsslError *error)
{
	...
	while((int)totalBytes < max_len)
	{
		...
		else if (numBytes == 0)
		{
			...
			totalBytes += numBytes;
#else
		numBytes = SOCK_RECV((RsslSocket)(intptr_t)transport, (buf + totalBytes), (max_len - totalBytes), 0);
...
//int ripcWriteV( void *transport, ripcIovType *iov, int iovcnt, int outLen, ripcRWFlags flags, RIPC_ERROR *error)
...
//int ripcWrite( void *transport, char *buf, int outLen, ripcRWFlags flags, RIPC_ERROR *error)
...
//static int ripcShutdownSckt(void *transport)
...
//int ripcScktReconnectClient(void *transport, RIPC_ERROR *error)
...
RsslInt32 ipcSrvrBind(rsslServerImpl *srvr, RsslError *error)
{
	...
	if ((portnum = ripcGetServByName(rsslServerSocketChannel->serverName)) == -1)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
	...
	if (ripcBindSocket(addr, portnum, sock_fd) < 0)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
...
RsslSocket ipcSrvrAccept(rsslServerImpl *srvr, void** userSpecPtr, RsslError *error)
{
	...
	fdtemp = accept(rsslServerSocketChannel->stream, (struct sockaddr *)0, (int *)0);
...
RsslSocket ipcConnectSocket(RsslInt32 *portnum, void *opts, RsslInt32 flags, void** userSpecPtr, RsslError *error)
{
	...
	{
		if ((*portnum = ripcGetServByName(pRsslSocketChannel->proxyPort)) == -1)
		{
			_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> Error: 1004 ripcGetServByName() failed. Port name/number is incorrect. System errno: (%d)\n",
				__FILE__, __LINE__, errno);
	...
	{
		if ((*portnum = ripcGetServByName(pRsslSocketChannel->serverName)) == -1)
		{
			_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> Error: 1004 ripcGetServByName() failed. Port name/number is incorrect. System errno: (%d)\n",
	...
	if (ripcBindSocket(localAddr, localPort, sock_fd) < 0)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
...
int ipcSetSockFuncs()
{
	...
	return(ripcSetTransFunc(RSSL_CONN_TYPE_SOCKET, &func));
}
 ...
 *		   -1 if error
 */
int ripcReadyWrite(RsslSocket fd)
{
	int				retval;
 ...
 *		   -1 if error
 */
int ripcReadyException(RsslSocket fd)
{
	int				retval;
