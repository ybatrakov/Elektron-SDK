...
RsslInt32 ripcInitializeSSL(char* libsslName, char* libcryptoName)
{
	...
	if((supportedProtocols & RIPC_PROTO_SSL_TLS_V1) != 0)
	{
		...
		retVal = ripcSetSSLTransFunc(RIPC_SSL_TLS_V1, &SSLfuncs);
		if(retVal == 0)
			goto cryptoLoadError;
	}
	...
	if((supportedProtocols & RIPC_PROTO_SSL_TLS_V1_1) != 0)
	{
		...
		retVal = ripcSetSSLTransFunc(RIPC_SSL_TLS_V1_1, &SSLfuncs);
		if(retVal == 0)
			goto cryptoLoadError;
	}
	...
	if((supportedProtocols & RIPC_PROTO_SSL_TLS_V1_2) != 0)
	{
		...
		retVal = ripcSetSSLTransFunc(RIPC_SSL_TLS_V1_2, &SSLfuncs);
		if(retVal == 0)
			goto cryptoLoadError;
	}
...
void ripcSSLErrors(RsslError *error, RsslInt32 initPos)
{
	...
	while ((l=(*(cryptoFuncs.get_error_line_data))(&file, &line, &data, &flags)) != 0)
	{
		...
		snprintf(error->text + cUrl, MAX_RSSL_ERROR_TEXT,  "%lu:%s:%s:%d:\n", es, buf, file, line);
...
SSL_CTX* ripcSSLSetupCTX(RsslInt32 user, ripcSSLProtocolFlags version, ripcSSLConnectOpts *config, RsslError *error)
{
		...
		if ((ctx = (*(ctxFuncs.ctx_new))( (*(sslFuncs.TLSv1_client_method))())) == NULL)
	{
		...
		snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> ripcSSLSetupCTX() failed to create new context", __FILE__,__LINE__,errno);
		ripcSSLErrors(error, strlen(error->text));
		return NULL;
	}
		break;
	...
	*/
	...
	if ((retVal = (*(ctxFuncs.ctx_set_cipher_list))(ctx, CIPHER_LIST)) != 1)
	{
		/* populate error and return failure */
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,"<%s:%d> ripcSSLSetupCTX() error setting up cipher list (no valid ciphers)", __FILE__,__LINE__,errno);
		ripcSSLErrors(error, strlen(error->text));
		goto ripcsetupctxend;
	}

	// BDS - need to get CAFILE or CADIR from config 
	if (config->CAfile || config->CApath)
	{
		if ((retVal = (*(ctxFuncs.ctx_load_verify_location))(ctx, config->CAfile, config->CApath)) != 1)
		{
			/* populate error and return failure */
			_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
			snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> ripcSSLSetupCTX() error loading CA file", __FILE__,__LINE__,errno);
			ripcSSLErrors(error, strlen(error->text));
			goto ripcsetupctxend;
		}
	...
	/* BDS - need to get CERTFILE from config */
	if (config->cert_file)
	{
	...
	while(totalBytes < max_len)
	{
		...
		switch ((*(sslFuncs.get_error))(sess->connection, numBytes))
		{
			...
			case SSL_ERROR_SYSCALL:
				/* OpenSSL does provide a way to get last error(get_last_socket_error(), but it's the same as the RIPC abstraction */
...
RsslInt32 ripcSSLAccept(void *session, ripcSessInProg *inPr, RsslError *error)
{
	...
	if (sess->server)
	{
		...
		{
			/* what do we do here? */
			/* BDS - do we need to do something here */
			return 0;
		}
	}
	else
...
{
	/* BDS - how do we pass in the connect options here??? */
