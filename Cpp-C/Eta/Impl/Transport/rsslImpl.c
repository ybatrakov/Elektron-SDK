...
static rtr_qtool_list freeChannelList;
static rtr_qtool_list freeServerList;
static rtr_qtool_list activeServerList;
static rtr_qtool_list activeChannelList;

/* used to help out with initialization and clean up */
static short numInitCalls = 0;
...
static RTR_STATIC_MUTEX_DECL(s_rsslStaticMutex);

RTR_C_ALWAYS_INLINE void _rsslStaticMutexLock()
{
	RTR_STATIC_MUTEX_LOCK( s_rsslStaticMutex );
}

RTR_C_ALWAYS_INLINE void _rsslStaticMutexUnlock()
{
	RTR_STATIC_MUTEX_UNLOCK( s_rsslStaticMutex );
}
...
RTR_C_ALWAYS_INLINE rsslChannelImpl *_rsslCreateChannel()
{
	...
	{
		RTR_MUTEX_INIT( chnl->chanMutex, mterr );
	}
	RTR_MUTEX_INIT( chnl->traceMutex, mterr );
	
	rtr_qtool_init(&(chnl->activeBufferList), sizeof(RsslBuffer));
	rtr_qtool_init(&(chnl->freeBufferList), sizeof(RsslBuffer));
	...
	for(i = 0; i < 10; i++)
	{
		if ((buffer = _rsslCreateBuffer(chnl)) != 0)
		{
			...
			rtr_qtool_insqt(&(chnl->freeBufferList), buffer);
		}
	}

	rtr_qtool_headInit(&freeChannelList, chnl);
...
RTR_C_ALWAYS_INLINE rsslServerImpl* _rsslCreateServer()
{
	...
	if (multiThread == RSSL_LOCK_GLOBAL_AND_CHANNEL)
	{
		RTR_MUTEX_INIT( srvr->srvrMutex, mterr );
	}
...
rsslChannelImpl *_rsslNewChannel()
{
	...
	if ((chnl = (rsslChannelImpl*)rtr_qtool_first(&freeChannelList)) == 0)
		chnl = _rsslCreateChannel();
	else
	{
		if (rtr_qtool_headActive(&freeChannelList, chnl))
		{
			rtr_qtool_remqe(&freeChannelList, chnl);
			if (memoryDebug)
...
rsslServerImpl *_rsslNewServer()
{
	...
	if ((srvr = (rsslServerImpl*)rtr_qtool_first(&freeServerList)) == 0)
		srvr = _rsslCreateServer();
	else
	{
		if (rtr_qtool_headActive(&freeServerList, srvr))
		{
			rtr_qtool_remqe(&freeServerList, srvr);
			if (memoryDebug)
...
RTR_C_ALWAYS_INLINE void _rsslReleaseActiveBuffers(rsslChannelImpl *chnl)
{
	...
	while (rsslBufImpl = rtr_qtool_takeqt(&(chnl->activeBufferList)))
	{
...
void RTR_FASTCALL _rsslReleaseChannel(rsslChannelImpl *chnl)
{
	...
	rtr_hashtiter_init(&iter, &(chnl->assemblyBuffers));

	for (rtr_hashtiter_start(&iter);
		!rtr_hashtiter_off(&iter);
		rtr_hashtiter_forth(&iter))
	{
		rsslAssemblyBuf = (rsslAssemblyBuffer*)rtr_hashtiter_item(&iter);
	...
	_rsslCleanChan(chnl);	
	mutexFuncs.staticMutexLock();
	if (rtr_qtool_headActive(&activeChannelList, chnl))
	{
		rtr_qtool_remqe(&activeChannelList, chnl);
		if (memoryDebug)
			printf("removing from activeChannelList\n");
	}

	rtr_qtool_headInit(&freeChannelList, chnl);
	rtr_qtool_insqt(&freeChannelList, chnl);
	if (memoryDebug)
...
void RTR_FASTCALL _rsslReleaseServer(rsslServerImpl *srvr)
{
	...
	if (srvr->transportInfo)
	{
		...
		retVal = ipcShutdownServer(srvr->transportInfo, &error);
		retVal = ipcSrvrDropRef(srvr->transportInfo, &error);
...
void _rsslCleanUp()
{
	...
	rsslServerImpl *srvr=0;
	rsslBufferImpl *buffer=0;
	rsslAssemblyBuffer *assemblyBuf=0;
	rtr_mutex_error mterr = 0;
	...
	while (chnl = (rsslChannelImpl *)rtr_qtool_takeqt(&activeChannelList))
	{
		_rsslReleaseChannel(chnl);
	}

	while (srvr = (rsslServerImpl *)rtr_qtool_takeqt(&activeServerList))
	{
		_rsslReleaseServer(srvr);
	}

	mutexFuncs.staticMutexLock();
	while (chnl = (rsslChannelImpl *)rtr_qtool_takeqt(&freeChannelList))
	{
		if (memoryDebug)
			printf("cleaning up free channel list\n");
	
		/* Channel has a free buffer list - clean this up */
		while (buffer = (rsslBufferImpl *)rtr_qtool_takeqt(&(chnl->freeBufferList)))
		{
		if (memoryDebug)
		...
		/* destroy the mutex */
		if (multiThread == RSSL_LOCK_GLOBAL_AND_CHANNEL)
			RTR_MUTEX_DESTROY(chnl->chanMutex, mterr);

		RTR_MUTEX_DESTROY(chnl->traceMutex, mterr);
	...
	while (srvr = (rsslServerImpl *)rtr_qtool_takeqt(&freeServerList))
	{
		if (memoryDebug)
		...
		/* destroy the mutex */
		if (multiThread == RSSL_LOCK_GLOBAL_AND_CHANNEL)
			RTR_MUTEX_DESTROY(srvr->srvrMutex, mterr);

		if (srvr->hasSharedBufPool)
		{
			RTR_MUTEX_DESTROY(srvr->sharedBufPoolMutex, mterr);
			srvr->hasSharedBufPool = RSSL_FALSE;
		}
...
{
	rtr_mutex_error mterr = 0;
	RTR_MUTEX_LOCK(rsslChnlImpl->traceMutex, mterr);
	...
	RTR_MUTEX_UNLOCK(rsslChnlImpl->traceMutex, mterr);
}
...
void _rsslTraceStartMsg(rsslChannelImpl *rsslChnlImpl, RsslBuffer *buffer, RsslRet *retTrace, traceOperation op, RsslError *error)
{
	...
	if(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr != NULL)
	{
		...
		if((filePos >= rsslChnlImpl->traceOptionsInfo.traceOptions.traceMsgMaxFileSize) && (rsslChnlImpl->traceOptionsInfo.traceOptions.traceFlags & RSSL_TRACE_TO_MULTIPLE_FILES))
		{
			unsigned long long hour = 0 , min = 0, sec = 0, msec = 0;
			char timeVal[33];
			int numChars = 0;

			/* Close this file, and open a new unique one */
			...
			numChars = snprintf(timeVal, 33,"%03llu.xml\0", msec);
			...
			if((rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr = fopen(rsslChnlImpl->traceOptionsInfo.newTraceMsgFileName, "a+")) == NULL)
			{
	...
	{
		snprintf(message, sizeof(message), "Incoming FD Change (Channel IPC descriptor = %d)", rsslChnlImpl->Channel.socketId);
		if(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr != NULL)
		{
	...
	{
		if (op == traceRead)
			snprintf(message, sizeof(message), "Incoming Message (Channel IPC descriptor = %d)", rsslChnlImpl->Channel.socketId);
		else if (op == traceWrite)
			snprintf(message, sizeof(message), "Outgoing Message (Channel IPC descriptor = %d)", rsslChnlImpl->Channel.socketId);
		else if (op == tracePack)
			snprintf(message, sizeof(message), "Pack Message (Channel IPC descriptor = %d)", rsslChnlImpl->Channel.socketId);
	...
	if (!isRead)
	{
		switch(*retTrace)
		{
			case RSSL_RET_FAILURE:
				snprintf(message, sizeof(message), "rsslWrite Failed (Channel IPC descriptor = %d)", rsslChnlImpl->Channel.socketId);
				if(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr != NULL)
				{
					xmlDumpComment(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr, message);
				}
			...
			case RSSL_RET_WRITE_CALL_AGAIN:
				snprintf(message, sizeof(message), "rsslWrite returned RSSL_RET_WRITE_CALL_AGAIN (Channel IPC descriptor = %d)", rsslChnlImpl->Channel.socketId);
				if(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr != NULL)
				{
					xmlDumpComment(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr, message);
				}
			...
			case RSSL_RET_INIT_NOT_INITIALIZED:
				snprintf(message, sizeof(message), "rsslWrite returned RSSL_RET_INIT_NOT_INITIALIZED (Channel IPC descriptor = %d)", rsslChnlImpl->Channel.socketId);
				if(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr != NULL)
				{
					xmlDumpComment(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr, message);
				}
		...
		snprintf(message, sizeof(message), "End Message (Channel IPC descriptor = %d)", rsslChnlImpl->Channel.socketId);
		if(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr != NULL)
		{
			xmlDumpComment(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr, message);
		}
...
void _rsslTraceClosed(rsslChannelImpl *rsslChnlImpl, RsslRet *retTrace)
{
	if (*retTrace == RSSL_RET_FAILURE && rsslChnlImpl->Channel.state == RSSL_CH_STATE_CLOSED)
	{
		...
		snprintf(message, sizeof(message), "Channel Closed (Channel IPC descriptor = %d)", rsslChnlImpl->Channel.socketId);
		RTR_MUTEX_LOCK(rsslChnlImpl->traceMutex, mterr);
		if(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr != NULL)
		{
			xmlDumpComment(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr, message);
...
RsslRet rsslInitializeEx(RsslInitializeExOpts *rsslInitOpts, RsslError *error)
{
	...
	if (!initialized)
	{
		...
		/* Initialize lists */
		rtr_qtool_init(&freeChannelList, sizeof(RsslChannel));
		rtr_qtool_init(&freeServerList, sizeof(RsslServer));
		rtr_qtool_init(&activeServerList, sizeof(RsslServer));
		rtr_qtool_init(&activeChannelList, sizeof(RsslChannel));
...
{
	char tempUserName[1024], *pTempUserName, *pBuf;
...
RsslServer* rsslBind(RsslBindOptions *opts, RsslError *error)
{
	...
	mutexFuncs.staticMutexLock();
	rtr_qtool_headInit(&activeServerList, rsslSrvrImpl);
	rtr_qtool_insqt(&activeServerList,  rsslSrvrImpl);
	if (memoryDebug)
...
RsslChannel* rsslAccept(RsslServer *srvr, RsslAcceptOptions *opts, RsslError *error)
{
	...
	mutexFuncs.staticMutexLock();
	rtr_qtool_headInit(&activeChannelList, rsslChnlImpl);
	rtr_qtool_insqt(&activeChannelList, rsslChnlImpl);
	if (memoryDebug)
		printf("adding chnl %d to activeChannelList\n", rsslChnlImpl->Channel.socketId);
	mutexFuncs.staticMutexUnlock();
...
RsslChannel* rsslConnect(RsslConnectOptions *opts, RsslError *error)
{
	...
	switch(opts->connectionType)
	{
	    ...
	    case RSSL_CONN_TYPE_RELIABLE_MCAST:
		{
			if (rsslLoadInitRsslTransportChannel(&(channelTransFuncs[RSSL_RRCP_TRANSPORT]), DEFAULT_RRCP_LIB_NAME, NULL) < 0)
			{
...
RsslRet rsslReconnectClient(RsslChannel *chnl,  RsslError *error)
{
	...
	/* Map RsslChannel to RIPC Socket */
...
RsslRet rsslInitChannel(RsslChannel *chnl, RsslInProgInfo *inProg, RsslError *error)
{
	...
	if ((!rsslChnlImpl->componentVer.componentVersion.length) && (!rsslChnlImpl->componentVer.componentVersion.data))
	{
		...
		else
		{
			...
			int defaultLength = RSSL_ComponentVersionStart_Len + RSSL_ComponentVersionEnd_Len - __RSZI8;
			int totalLength = rsslChnlImpl->connOptsCompVer.componentVersion.length + __RSZI8 + defaultLength;
			...
			MemCopyByInt(rsslChnlImpl->componentVer.componentVersion.data, rsslComponentVersionStart, RSSL_ComponentVersionStart_Len);
			length = RSSL_ComponentVersionStart_Len;
			...
			length += RSSL_ComponentVersionEnd_Len - __RSZI8;
	...
	{
		char message[128];
		snprintf(message, sizeof(message), "Connection Established (Channel IPC descriptor = %d)\0", rsslChnlImpl->Channel.socketId);
...
RsslRet rsslCloseChannel(RsslChannel *chnl, RsslError *error)
{
	...
	{
		char message[128];
		snprintf(message, sizeof(message), "rsslCloseChannel Connection closed (Channel IPC descriptor = %d)", rsslChnlImpl->Channel.socketId);
		if(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr != NULL)
		{
			_rsslXMLDumpComment(rsslChnlImpl, rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr, message);
		}
...
RsslRet rsslIoctl(RsslChannel *chnl, RsslIoctlCodes code, void *value, RsslError *error)
{
	...
	switch (code)
	{
		case RSSL_TRACE:
			...
			if(((RsslTraceOptions *)value) != NULL)
			{
				unsigned long long hour = 0 , min = 0, sec = 0, msec = 0;
				char timeVal[33];
				int numChars = 0;
				...
				rsslChnlImpl->traceOptionsInfo.traceOptions.traceFlags = ((RsslTraceOptions *)value)->traceFlags;
				rsslChnlImpl->traceOptionsInfo.traceOptions.traceMsgMaxFileSize = ((RsslTraceOptions *)value)->traceMsgMaxFileSize;

				/* check if file tracing is being disabled */
				if(!(((RsslTraceOptions *)value)->traceFlags & RSSL_TRACE_TO_FILE_ENABLE))
				{
					/* free memory and close file descriptors associated with the trace options */
					if(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr != 0)
					{
						fclose(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr);
						rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr = 0;
					}
					if(rsslChnlImpl->traceOptionsInfo.traceOptions.traceMsgFileName != NULL)
					{
						free(rsslChnlImpl->traceOptionsInfo.traceOptions.traceMsgFileName);
					...
					rsslChnlImpl->traceOptionsInfo.newTraceMsgFileName = NULL;
					rsslChnlImpl->traceOptionsInfo.traceMsgOrigFileNameSize = 0;
					return RSSL_RET_SUCCESS;
				}
				else if((((RsslTraceOptions *)value)->traceMsgFileName == NULL) && (((RsslTraceOptions *)value)->traceFlags & RSSL_TRACE_TO_FILE_ENABLE))
				{
					if (rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr == NULL)
					{
				...
				else if((((RsslTraceOptions *)value)->traceMsgFileName != NULL) && (rsslChnlImpl->traceOptionsInfo.traceOptions.traceMsgFileName != NULL))
				{
					if(!(strcmp(((RsslTraceOptions *)value)->traceMsgFileName, rsslChnlImpl->traceOptionsInfo.traceOptions.traceMsgFileName)))
					{
						/* the user wants to change the output file for the XML trace.
						free memory and close file descriptors associated with the old file name*/
						if(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr != 0)
						{
							fclose(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr);
						}
				...
				if(((RsslTraceOptions *)value)->traceMsgFileName != NULL)
				{
					rsslChnlImpl->traceOptionsInfo.traceMsgOrigFileNameSize = strlen(((RsslTraceOptions *)value)->traceMsgFileName);
					rsslChnlImpl->traceOptionsInfo.traceOptions.traceMsgFileName = (char*)malloc(rsslChnlImpl->traceOptionsInfo.traceMsgOrigFileNameSize + sizeof(char));
					...
					memcpy(rsslChnlImpl->traceOptionsInfo.traceOptions.traceMsgFileName, ((RsslTraceOptions *)value)->traceMsgFileName, rsslChnlImpl->traceOptionsInfo.traceMsgOrigFileNameSize);
					rsslChnlImpl->traceOptionsInfo.traceOptions.traceMsgFileName[rsslChnlImpl->traceOptionsInfo.traceMsgOrigFileNameSize] = '\0';
				}

				/* malloc space for the modified file name, which includes the original name and 33 additional chars to hold time stamps (when needed) 
				 * and the ".xml" extension*/
				rsslChnlImpl->traceOptionsInfo.newTraceMsgFileName = (char*)malloc(rsslChnlImpl->traceOptionsInfo.traceMsgOrigFileNameSize + 33*sizeof(char));
	...
	if (rtrUnlikely(rsslChnlImpl->traceOptionsInfo.traceOptions.traceFlags & (RSSL_TRACE_TO_FILE_ENABLE | RSSL_TRACE_TO_STDOUT)))
	{
		...
		{	
			if (rsslChnlImpl->traceOptionsInfo.traceOptions.traceFlags & RSSL_TRACE_PING)	/* are we tracing pings? */
			{
				...
				snprintf(message, sizeof(message), "Incoming Ping (Channel IPC descriptor = %d)", rsslChnlImpl->Channel.socketId);
				if(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr != NULL)
				{
					xmlDumpComment(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr, message);
					xmlDumpTimestamp(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr);
	...
	if (rtrUnlikely(rsslChnlImpl->traceOptionsInfo.traceOptions.traceFlags & (RSSL_TRACE_TO_FILE_ENABLE | RSSL_TRACE_TO_STDOUT)))
	{
		if(rsslChnlImpl->traceOptionsInfo.traceOptions.traceFlags & RSSL_TRACE_WRITE)
		{
			if (rsslChnlImpl->traceOptionsInfo.traceOptions.traceFlags & RSSL_TRACE_PING)	/* are we tracing pings? */
			{
				...
				snprintf(message, sizeof(message), "Outgoing Ping (Channel IPC descriptor = %d)\0", rsslChnlImpl->Channel.socketId);
				if(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr != NULL)
				{
					xmlDumpComment(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr, message);
					xmlDumpTimestamp(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr);
				...
				snprintf(message, sizeof(message), "End Message (Channel IPC descriptor = %d)\0", rsslChnlImpl->Channel.socketId);
				if(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr != NULL)
				{
					xmlDumpComment(rsslChnlImpl->traceOptionsInfo.traceMsgFilePtr, message);
				}
...
RSSL_API RsslBuffer* rsslGetBuffer(RsslChannel *chnl, RsslUInt32 size, RsslBool packedBuffer, RsslError *error)
{
	...
	rtr_qtool_headInit(&(rsslChnlImpl->activeBufferList), rsslBufImpl);
	rtr_qtool_insqt(&(rsslChnlImpl->activeBufferList), rsslBufImpl);
...
RSSL_API RsslUInt32 rsslCalculateHexDumpOutputSize(const RsslBuffer *bufferToHexDump, RsslUInt32 valuesPerLine)
{
	...
	return rtr_dumpHexSize(bufferToHexDump->data, bufferToHexDump->length, valuesPerLine, 0);
}
...
RSSL_API RsslRet rsslBufferToHexDump(const RsslBuffer* bufferToHexDump, RsslBuffer* hexDumpOutput, RsslUInt32 valuesPerLine, RsslError *error)
{
	...
	/* returns length used or less than zero if additional bytes are needed */
	outputLen = rtr_dumpHexToBuffer(hexDumpOutput->data, hexDumpOutput->length, bufferToHexDump->data, bufferToHexDump->length, valuesPerLine);
	...
	if (outputLen < 0)
	{
		...
		hexDumpOutput->length = outputLen;
