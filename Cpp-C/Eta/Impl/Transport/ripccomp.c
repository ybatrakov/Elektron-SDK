...
static RsslRet zlibcompress(void *zstream, ripcCompBuffer *buf, RsslError *error)
{
	...
	if (rtrUnlikely(compressionDebug)) printf("zlib Compressed %d inbytes to %d outbytes (avail_out = %d)\n", buf->bytes_in_used, buf->bytes_out_used, buf->avail_out);
...
RsslInt32 ripcInitZlibComp()
{
	...
	return(ripcSetCompFunc(RIPC_COMP_ZLIB,&funcs));
}
...
{
	// we dont need to do anything with the fake zstream that was returned by lz4CompInit()
}

static void lz4DecompEnd(void *zstream)
{
	// we dont need to do anything with the fake zstream that was returned by lz4DecompInit()
}
...
static RsslRet lz4Comp(void* stream, ripcCompBuffer *buf, RsslError *error)
{
	...
	if (compressionDebug) printf("LZ4 Compressed %d inbytes to %d outbytes (max = %d)\n", buf->avail_in, err, LZ4_compressBound(buf->avail_in));
	// 
...
static RsslRet lz4Decomp(void* stream, ripcCompBuffer *buf, RsslError *error)
{
	...
	if (compressionDebug) printf("LZ4 Decompressed %d inbytes to %d outbytes\n", buf->avail_in, err);
...
int ripcInitLz4Comp()
{
	...
	return(ripcSetCompFunc(RIPC_COMP_LZ4,&funcs));
}
