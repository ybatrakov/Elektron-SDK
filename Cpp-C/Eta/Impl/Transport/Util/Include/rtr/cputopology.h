...
enum {rtrIntelProcessor = 0, rtrAMDProcessor = 1, rtrOtherProcessor = 2};
...
//unsigned testmask();
//extern unsigned GetGenericAffinityMask(unsigned package, unsigned core, unsigned logical, unsigned int slen, char *sbuf);
...
//int rtrGetCpuAffinity(int package, int core, int thread);
unsigned rtrGetLogicalCpuCount();
unsigned rtrGetEnumeratedCoreCount(unsigned package_ordinal);
unsigned rtrGetEnumeratedThreadCount(unsigned package_ordinal, unsigned core_ordinal);
unsigned rtrGetProcessorPackageCount();
unsigned rtrBindThreadWithString(const char *cpuString, char *logicalOutputString);
unsigned rtrBindThread(unsigned cpu);
