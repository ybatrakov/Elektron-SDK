...
#include "rtr/cutilsmplcbuffer.h"	/* RDEVDIFFOK */
...
rtr_msgb_t *rtr_smplcIntAllocD(rtr_bufferpool_t *pool, size_t size)
{
	...
	rtr_msgb_t	*mblk=0;
	rtr_datab_t	*dblk;
	memory = PLATFORM_MEMORY_ALLOC(sizeof(rtr_datab_t) + sizeof(rtr_msgb_t) + rtr_cbufferCppOverhead + size);
	if (memory)
	{
...
{
	rtr_msgb_t	*mblk;
	mblk = (rtr_msgb_t*)PLATFORM_MEMORY_ALLOC(sizeof(rtr_msgb_t) + rtr_cbufferCppOverhead);
	if (mblk)
	{
		mblk->nextMsg = 0;
		mblk->buffer = 0;
...
int rtr_smplcFreeMsg(rtr_msgb_t *mblk)
{
	...
	{
		PLATFORM_MEMORY_DEALLOC(mblk->datab);
	}

	if (doFree)
	{
		PLATFORM_MEMORY_DEALLOC(mblk);
	}
...
rtr_bufferpool_t *rtr_smplcAllocatePool(rtr_mutex *mutex)
{
	rtr_bufferpool_t *retpool;

	retpool = (rtr_bufferpool_t*)PLATFORM_MEMORY_ALLOC(sizeof(rtr_bufferpool_t));
	if (retpool)
	{
		retpool->maxBufs = -1;
		retpool->numBufs = 0;
