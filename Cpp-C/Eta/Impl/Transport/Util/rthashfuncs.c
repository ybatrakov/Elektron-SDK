...
#ifdef WIN32
rtrUInt32 __int_rtr_hash_crc_table[256] = {
#else
rtrUInt32 __rtr_hash_crc_table[256] = {
#endif
...
#ifdef WIN32
rtrUInt32* __rtrGetHashCrcTable()
{
	return __int_rtr_hash_crc_table;
}
#endif

    /* CRC32 table for polynomial 0xF3C5F6A9 */

rtrHashSum rtrPolyHashFunc(const char* buf, const rtrUInt32 length)
{
	return rtrPolyHash(buf,length);
}

rtrHashSum rtrRTICHashFunc(const char* buf, const rtrUInt32 length)
{
	return rtrRTICHash(buf,length);
}

rtrHashSum rtrMDHHashFunc(const char* buf, const rtrUInt32 length)
{
	return rtrMDHHash(buf,length);
}

rtrHashSum rtrSDBMHashFunc(const char* buf, const rtrUInt32 length)
{
	return rtrSDBMHash(buf,length);
}

rtrHashSum trFNV1HashFunc(const char* buf, const rtrUInt32 length)
{
	return rtrFNV1Hash(buf,length);
}

rtrHashSum trFNV1aHashFunc(const char* buf, const rtrUInt32 length)
{
	return rtrFNV1aHash(buf,length);
}

/* Calculate the denominator used in the IDN Hashing entity ID
 * equation.
 */
unsigned long rtrHashDenominator(rtrInt32 numberOfHashingEntities)
{
	const unsigned long long TotalRange = 0x100000000ull;

	unsigned long denominator = TotalRange / numberOfHashingEntities;
...
rtrHashSum trHashingEntityId(const char *buf, const rtrUInt32 length, const rtrInt32 numberOfHashingEntities)
{
	rtrHashSum sum = rtrPolyHashFunc(buf, length) / rtrHashDenominator(numberOfHashingEntities);
