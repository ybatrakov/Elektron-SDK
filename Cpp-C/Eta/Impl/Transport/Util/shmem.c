...
/* On linux, this name becomes the file in /dev/shm(the shared memory folder). On solaris they are in /tmp and preceeded witha "."
 * The standard asks that the name is prepended with a '/'. */
const char *RTR_SHM_SEG_NAME = "/tr.seg.%s";
const int RTR_SHM_SEG_KEY_MAX_LEN = __MY_MAX_ADDR - 9; /* __MY_MAX_ADDR - <size of string without key> */
...
const int RTR_MTX_KEY_MAX_LEN = __MY_MAX_ADDR - 15; /* __MY_MAX_ADDR - <size of string without key + longest ushort(5)> */

/*** Named Pipe ***/

/* Like the shm segments, creation should fail if the file already exists */
const int RTR_NAMEDPIPE_CREATE_FLAGS = O_WRONLY /*O_RDWR /*| O_NONBLOCK */;
const int RTR_NAMEDPIPE_ATTACH_FLAGS = O_RDONLY/*| O_NONBLOCK*/;
...
rtrNamedSem *_rtrShmMutexOpenInt( int isEvent/* is event or mutex? */, int isOwner /* Create or attach? */, const char* key, unsigned short mutexNumber, char *errBuff )
{
	...
	if (!newMutex) 
	{ 
		snprintf(errBuff, __ERROR_LEN, "_rtrShmMutexOpenInt() newMutex malloc failed (size = %d, key = %s)", sizeof(rtrNamedSem), key);
		return 0; 
	}

	if (strlen(key) > RTR_MTX_KEY_MAX_LEN)
	{
		snprintf(errBuff, __ERROR_LEN, "_rtrShmMutexOpenInt() Illegal shared mutex key length %d (key = %s)",strlen(key), key);
		free(newMutex);
		return 0;
	}
	...
	{ 
		...
		if (!newMutex->name) 
		{ 
			snprintf(errBuff, __ERROR_LEN, "_rtrShmMutexOpenInt() newMutex->name malloc failed (size = %d, key = %s)", 1+strlen(memMapName), key);
			free(newMutex); 
			return 0; 
		}
...
#ifdef _WIN32
{
	...
	{
		snprintf(errBuff, __ERROR_LEN, "rtrShmSegCreate() Illegal shared memory key length %d (key = %s)",strlen(key), key);
		return -1;
	}
...
#else
{
	...
	{ 
		snprintf(errBuff,__ERROR_LEN, "rtrShmSegCreate() mlock() Failed (errno = %d, mapName = %s, size = %d)", errno, memMapName, size);
		rtrShmSegDestroy(seg); 
		return -1;
	}
	...
	seg->hdr = (rtrShmHdr*)seg->base;
	seg->hdr->magic = RTR_SHM_MAGIC;
	seg->hdr->creatorPid = (unsigned short)getpid(); /* TODO JimC: Don't know if a short is ok. */
	seg->hdr->totalLen = size;
	seg->hdr->headerLen = (unsigned short)RTR_SHM_ALIGNBYTES(sizeof(rtrShmHdr));
	seg->hdr->freeOffset = seg->hdr->headerLen;
	...
	if (!seg->hdr->name) 
	{ 
		snprintf(errBuff,__ERROR_LEN, "rtrShmSegCreate() malloc() Failed (mapName = %s, size = %d)", memMapName, size);
		rtrShmSegDestroy(seg); 
		return -1; 
	}
...
#else
{
	if (seg->base && seg->hdr)
	{
		if (seg->hdr->name) { shm_unlink(seg->hdr->name); seg->hdr->name = 0; }
...
#ifdef _WIN32
{
	...
	if (strlen(key) > (__MY_MAX_ADDR - 6))
	{
		snprintf(errBuff,__ERROR_LEN, "rtrShmSegAttach() Illegal shared memory key length %d (key = %s)", strlen(key), key);
		return -1;
	}
...
#else
{
	...
	if (strlen(key) > RTR_SHM_SEG_KEY_MAX_LEN)
	{
		snprintf(errBuff,__ERROR_LEN, "rtrShmSegAttach() Illegal shared memory key length %d %s",strlen(key), key);
		return -1;
	}
	...
	if (seg->hdr->magic != RTR_SHM_MAGIC)
	{
		snprintf(errBuff,__ERROR_LEN, "rtrShmSegAttach() Illegal Shared Memory Magic 0x%x\n",seg->hdr->magic);
		return -1;
	}
...
#ifdef _WIN32
{
	...
	{
		snprintf(errBuff,__ERROR_LEN, "rtrShmSegCreateEvent() Illegal shared event key length %d (key = %s)",strlen(key), key);
		return 0;
	}
...
#ifdef _WIN32
{
	...
	if (strlen(key) > (__MY_MAX_ADDR - 10))
	{
		snprintf(errBuff,__ERROR_LEN, "rtrShmSegAttachEvent() Illegal shared event key length %d (key = %s)",strlen(key), key);
		return 0;
	}
...
#ifdef _WIN32
{
	...
	if (strlen(key) > (__MY_MAX_ADDR - 10))
	{
		snprintf(errBuff,__ERROR_LEN, "rtrShmSegCreateMutex() Illegal shared mutex key length %d (key = %s)",strlen(key), key);
		return 0;
	}
...
#ifdef _WIN32
{
	...
	if (strlen(key) > (__MY_MAX_ADDR - 10))
	{
		snprintf(errBuff,__ERROR_LEN, "rtrShmSegAttachMutex() Illegal shared mutex key length %d (key = %s)",strlen(key), key);
		return 0;
	}
...
rtrShmNamedPipePtr rtrShmSegCreateNamedPipe( const char *key, unsigned short namedPipeNumber, char *errBuff )
{
	...
	{
		snprintf(errBuff,__ERROR_LEN, "rtrShmSegCreateNamedPipe() Illegal named pipe key length %d (key = %s)",strlen(key), key);
		return 0;
	}

	snprintf(namedPipeName,__MY_MAX_ADDR, RTR_NAMEDPIPE_NAME, key, namedPipeNumber );	//TODO: fix the name
	fprintf(stderr, "*** creating named pipe called %s\n", namedPipeName);
...
//	fprintf(stderr, "*** rtrShmSegAcceptNamedPipe called for %s with open flags %x\n", namedPipe->name, RTR_NAMEDPIPE_CREATE_FLAGS);
	...
	// TODO: do a blocking accept (for the prototype)
	// TODO: maybe I should check the return code of the F_GETFD
...
//	fprintf(stderr, "*** rtrShmSegAcceptNamedPipe returning (flags = 0x%x)\n", fcntl(acceptFD, F_GETFL, 0));
	return acceptFD;
}
...
rtrShmNamedPipePtr rtrShmSegAttachNamedPipe( const char *key, unsigned short namedPipeNumber, char *errBuff )
{
	...
	if (strlen(key) > (__MY_MAX_ADDR - 20))
	{
		snprintf(errBuff,__ERROR_LEN, "rtrShmSegAttachNamedPipe() Illegal shared mutex key length %d (key = %s)",strlen(key), key);
		return 0;
	}
...
//	fprintf(stderr, "*** rtrShmSegBindSocketTCP called for port %s\n", portNumber);

	// TODO: may need to initialize sockets on windows
...
//	fprintf(stderr, "*** rtrShmSegAcceptSocketTCP called for FD %d\n", serverFD);

	if ((acceptFD = accept( serverFD, (struct sockaddr *)&cliSAddr, &SAddrSize )) == -1)
	{
...
//	fprintf(stderr, "*** rtrShmSegAcceptSocketTCP returning (flags = 0x%x)\n", fcntl(acceptFD, F_GETFL, 0));
	return acceptFD;
}
...
//	fprintf(stderr, "*** rtrShmSegAttachSocketTCP called for port %s\n", portNumber);
...
//	fprintf(stderr, "*** rtrShmSegAttachSocketTCP returning (flags = 0x%x)\n", fcntl(clientFD, F_GETFL, 0));
	return clientFD;
}
...
//	fprintf(stderr, "*** rtrShmSegBindSocketUDP called for port %s\n", portNumber);
...
//	fprintf(stderr, "*** rtrShmSegAcceptSocketUDP called for FD %d\n", serverFD);
	
	if ((count = recvfrom(serverFD, buffer, sizeof(buffer), /* MSG_DONTWAIT */0, (struct sockaddr *) cliAddr, cliLen)) == -1)
	{
...
//	printf("listener: got packet from %x/%d\n",cliAddr->sin_addr.s_addr, cliAddr->sin_port);
//	printf("count of bytes read in rtrShmSegAcceptSocketUDP = %d\n", count);
	return 0;
}
...
//	fprintf(stderr, "*** rtrShmSegAttachSocketUDP called for port %s\n", portNumber);
...
//	if (sendto(clientFD, "1", 1, 0, (struct sockaddr *)&srvrSAddr, sizeof(srvrSAddr)) == -1)
	if (send(clientFD, "1", 1, 0) == -1)
	{
		snprintf(errBuff,__ERROR_LEN, "rtrShmSegAttachSocketUDP() socket sendto failed (errno = %d)", errno);
		return -1;
	}
	//TODO: do a blocking connect (for the prototype)
#if WIN32
	if (ioctlsocket( clientFD, FIONBIO, &arg ) == -1)
#else
	if (fcntl( clientFD, F_SETFL, fcntl(clientFD, F_GETFL, 0) | O_NONBLOCK | O_NDELAY ) == -1)
...
//	fprintf(stderr, "*** rtrShmSegAttachSocketUDP returning (flags = 0x%x)\n", fcntl(clientFD, F_GETFL, 0));
	return clientFD;
}
