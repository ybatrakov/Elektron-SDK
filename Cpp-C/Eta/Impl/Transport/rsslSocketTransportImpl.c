...
static ipcSessInit ipcReadHdr(RsslSocketChannel*, ripcSessInProg*, RsslError*);
static ipcSessInit ipcInitTransport(RsslSocketChannel*, ripcSessInProg*, RsslError*);
static ipcSessInit ipcInitTransport(RsslSocketChannel*, ripcSessInProg*, RsslError*);
static ipcSessInit ipcInitClientTransport(RsslSocketChannel*, ripcSessInProg*, RsslError*);
static ipcSessInit ipcInitClientTransport(RsslSocketChannel*, ripcSessInProg*, RsslError*);
static ipcSessInit ipcFinishSess(RsslSocketChannel*, ripcSessInProg*, RsslError*);
static ipcSessInit ipcWaitProxyAck(RsslSocketChannel*, ripcSessInProg*, RsslError*);
static ipcSessInit ipcWaitClientKey(RsslSocketChannel*, ripcSessInProg*, RsslError*);
static ipcSessInit ipcSendClientKey(RsslSocketChannel*, ripcSessInProg*, RsslError*);
static ipcSessInit ipcRejectSession(RsslSocketChannel*, RsslUInt16, RsslError*);
static ipcSessInit ipcConnecting(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error);
static ipcSessInit ipcProxyConnecting(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error);
static ipcSessInit ipcClientAccept(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error);
static ipcSessInit ipcReconnectSocket(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error);
ipcSessInit ipcWaitProxyAck(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error);
RsslRet ipcIntWrtHeader(RsslSocketChannel *rssl, RsslError *error);
ipcSessInit ipcSessionInit(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error);
RsslRet ripcSessDropRef(RsslSocketChannel *rsslSocketChannel, RsslError *error);
RsslRet ipcShutdownSockectChannel(RsslSocketChannel* rsslSocketChannel, RsslError *error);
RsslRet ipcShutdownSession(RsslSocketChannel* rsslSocketChannel, RsslError *error);
...
//static rtr_qtool_list	freeServerList;
//static rtr_qtool_list	freeUserServerList;
//static rtr_qtool_list	freeSessionList;
//static rtr_qtool_list	freeUserSessionList;


/* Used to tell if openSSL has been loaded */
#ifndef _WIN32
static RsslUInt8 openSSLInit = 0;
static char* libsslConfigName = NULL;
static char* libcryptoConfigName = NULL;
#endif
...
* type of compression.
*/
static ripcCompFuncs	compressFuncs[RIPC_COMP_MAX_TYPE + 1];

static const RsslUInt32	RIPC_COMP_DFLT_THRESHOLD_ZLIB = 30;
static const RsslUInt32	RIPC_COMP_DFLT_THRESHOLD_LZ4 = 300;
...
static ipcSSLFuncs		SSLTransFuncs;

static RsslUInt16		numInitCalls = 0;
static RsslUInt8 		initialized = 0;
static RsslUInt8		gblmutexinit = 0;

/* winInet tunneling */
#include "rtr/ripcinetutils.h"

extern rtr_mutex		ripcMutex;
extern rtr_qtool_list	activeSessionList;
...
///* converts RIPC_ERROR to RsslError */
//RTR_C_ALWAYS_INLINE void _rsslErrorMapping(RsslError *error, RsslChannel *chnl, RIPC_ERROR *ripc_error)
//{
//	error->channel = chnl;
//	error->sysError = ripc_error->sys_errno;
//	snprintf(error->text, MAX_RSSL_ERROR_TEXT, "%s", ripc_error->errorText);
//
//	/* need to convert the ripc_error to rsslErrorId */
//	/* we may want to expand the error returning of rssl */
//		
//	switch (ripc_error->ripc_errno)
//	{
//		case RIPC_ERR:
//		case RIPC_SYS_ERR:
//		case RIPC_INSUF_MEMORY:
//		case RIPC_BADARG:
...
void setRipcError(RIPC_ERROR *error, RIPC_SOCKET *sck, ripcError serr, RsslInt32 syserr)
{
	error->socket = sck;
	error->ripc_errno = serr;
#if defined(_WIN32)
	if (syserr)
		error->sys_errno = syserr;
	else
		error->sys_errno = GetLastError();
#else
	error->sys_errno = syserr;
#endif
}

RsslRet ripcSetCompFunc(RsslInt32 compressionType, ripcCompFuncs *funcs)
{
	if (compressionType > RIPC_COMP_MAX_TYPE)
		return(-1);
...
RsslRet ripcSetTransFunc(RsslInt32 type, ripcTransportFuncs *funcs)
{
...
RsslRet ripcSetSSLTransFunc(RsslInt32 type, ripcTransportFuncs *funcs)
{
...
RsslRet ripcSetSSLFuncs(ipcSSLFuncs *funcs)
{
	...
	*/
	...
	printf("LOCK rsslSocketChannel -- ipcReadSession (after transportFuncs->readTransport)\n");
		...
		if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
		{
			...
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> Error: 1003 ipcReadSession() failed due to session shutting down.\n",
				__FILE__, __LINE__);
...
#ifdef IPC_DEBUG
		if (rtrUnlikely(readdebug))
			printf("ipcReadSession() fd %d cc %d err %d\n",
		...
		if (cc < 0)
		{
			...
			if ((rsslSocketChannel->newStream != RIPC_INVALID_SOCKET) && (rsslSocketChannel->tunnelingState == RIPC_TUNNEL_ACTIVE))
			{
				...
				rsslSocketChannel->transportInfo = (void*)(intptr_t)(rsslSocketChannel->stream);
				sock_close(rsslSocketChannel->oldStream);
	...
	printf("LOCK rsslSocketChannel -- ipcReadSession (after transportFuncs->readTransport)\n");
		...
		if ((ipcLen = (RsslUInt32)messageLength) > (RsslUInt32)rsslSocketChannel->maxMsgSize)
		{
			_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> Error: 1007 Invalid Message Size. Message size is: (%d). Max Message size is(%d)\n",
	...
	/* dont move input buf cursor yet, moved below */
	...
	if (ipcOpcode & IPC_COMP_DATA)
	{
		...
		* and points the compression at the right place to do decompression */
		...
		if ((rsslSocketChannel->inDecompress == RIPC_COMP_LZ4) && ((rsslSocketChannel->tempDecompressBuf->length) || (ipcOpcode & IPC_COMP_FRAG)))
		{
			...
			{
				/* first part, copy it and dont decompress */
			...
			else
			{
				...
				compBuf.avail_in = rsslSocketChannel->tempDecompressBuf->length;
				compBuf.next_out = rsslSocketChannel->decompressBuf->buffer + rsslSocketChannel->decompressBuf->length;
				compBuf.avail_out = rsslSocketChannel->decompressBuf->maxLength - rsslSocketChannel->decompressBuf->length;
		...
		*moreData = (RsslInt32)ipcOpcode;
		...
		/* return WOULD_BLOCK to let the user know
		we need to read more to complete the message */
...
if (IPC_NULL_PTR(rsslSocketChannel, "ipcFragmentationDataBuffer", "rsslSocketChannel", error))
	...
	{
		neededSize = dataSize + SUBSEQ_FRAG_HEADER_SIZE < rsslSocketChannel->maxMsgSize ? dataSize + SUBSEQ_FRAG_HEADER_SIZE : rsslSocketChannel->maxMsgSize;
	}
...
rtr_bufferpool_t *ripcCreatePool(RsslInt32 max_bufs, rtr_mutex *mutex)
{
...
RsslInt32 ipcIntWrtHeader(RsslSocketChannel *rsslSocketChannel, RsslError *error)
{
	...
	if (rtrUnlikely(conndebug))
		printf("ripcIntWrtHeader() called\n");
#endif
	if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1003 ipcIntWrtHeader() failed due to session shutting down.\n",
			__FILE__, __LINE__);
	...
	lastmb = (rtr_msgb_t*)rtr_qtool_last(&rsslSocketChannel->priorityQueues[0].priorityQueue);

	if (lastmb && ((lastmb->buffer + lastmb->length) == buffer->buffer))
	{
	...
	else
	{
		...
		rtr_qtool_insqt(&(rsslSocketChannel->priorityQueues[0].priorityQueue), buffer);
	}
	...
	RsslInt32 *uncompBytesWritten, RsslInt32 forceFlush, RsslError *error)
{
	...
	RsslUInt8		fragmentationId = 0;
	RsslInt32		totalLength = 0;
	RsslInt32		fragHeaderSize = 0;
	RsslInt32		NumPkts = 0;
	RsslUInt32		outBytes = 0;
	RsslInt32		NumPktsRemainder = 0;
	RsslInt32		uncompBytes = 0;
	RsslInt32		chunkLen = 0;
	RsslUInt16		httpHeaderLen = 0;
	RsslUInt16		tempLen;
	rtr_msgb_t		*compressedmb1;
	rtr_msgb_t		**msgb = (rtr_msgb_t**)(&rsslBufferImpl->bufferInfo);
	...
	if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1003 ipcIntWrtSess() failed due to session shutting down.\n",
			__FILE__, __LINE__);
	...
	while (*msgb)
	{
		...
		if ((*msgb)->length > 0)
		{
				...
				*/
				...
				if (compressedmb1 == 0)
				{
#ifdef IPC_DEBUG
					if (rtrUnlikely(readdebug))
						printf("ipcWriteSession NOT doing compression (flags=0x%x, wFlags=0x%x, opCodes=0x%x, fragId=0x%x, len=%d)\n",
					...
					if (rsslSocketChannel->httpHeaders)
					{
						...
						trouble */
						/* dont want this length to include the http header */
					...
					/* Change header */
					/* we dont want any http header info included in this length */
				...
				else /* _conn_version_10/11/12/13 && compression */
				{
					...
					/* dont want it to have any http headers */
					tempLen = messageLength - httpHeaderLen;
					RTR_PUT_16(hdr, tempLen);
					...
					if ((rsslSocketChannel->outCompression == RIPC_COMP_LZ4) && (tempLen >= rsslSocketChannel->upperCompressionThreshold))
					{
						compBuf.next_out = rsslSocketChannel->tempCompressBuf->buffer;
						compBuf.avail_out = rsslSocketChannel->tempCompressBuf->maxLength;
					}
					else
					{
						compBuf.next_out = compressedmb1->buffer + headerLength;
						compBuf.avail_out = (compressedmb1->maxLength - headerLength - footer_size);
					}
					...
					if ((rsslSocketChannel->outCompression == RIPC_COMP_LZ4) && (tempLen >= rsslSocketChannel->upperCompressionThreshold))
					{
						...
						if (rsslSocketChannel->tempCompressBuf->length <= compLen1)
						{
							...
							/* reset length for reuse,  since this doesnt span two buffers we dont need it */
					...
					if (compBuf.avail_out == 0)
					{
						...
						if (rsslSocketChannel->httpHeaders)
						{
							...
							/* add the carrage return line feed */
							IPC_ADD_CR_LF(compressedmb1, chunkLen);
						...
						if (forceFlush == 1)
						{
							...
							if (!queuedBytesToWrite)
							{
								...
								if (rsslSocketChannel->httpHeaders)
									cc = (*(rsslSocketChannel->transportFuncs->writeTransport))(rsslSocketChannel->tunnelStreamFd, compressedmb1->local, lenToWrite, rwflags, error);
								else
								...
								if ((cc < 0) && (rsslSocketChannel->newTunnelStreamFd) && (rsslSocketChannel->httpHeaders))
								{
								...
								if (cc == lenToWrite)
								{
									...
									if (rsslSocketChannel->newTunnelStreamFd && rsslSocketChannel->httpHeaders  && rsslSocketChannel->sentControlAck)
									{
										RsslInt32 outLen;
										...
										writtenLen = (*(rsslSocketChannel->transportFuncs->writeTransport))(rsslSocketChannel->tunnelStreamFd, outBuf, outLen, rwflags, error);
										...
										if (writtenLen == outLen)
										{
											...
											shutdown((RsslSocket)(intptr_t)(rsslSocketChannel->tunnelStreamFd), shutdownFlag);

											if ((rsslSocketChannel->oldTunnelStreamFd) && (rsslSocketChannel->oldTunnelStreamFd != rsslSocketChannel->newTunnelStreamFd))
											...
											* Otherwise the shutdown() call will have already closed the socket properly. */
											if (!rsslSocketChannel->isJavaTunnel)
												rsslSocketChannel->oldTunnelStreamFd = rsslSocketChannel->tunnelStreamFd;
						...
						if ((forceFlush == 0) || (forceFlush == 2))
						{
							...
							rtr_qtool_insqt(&(rsslSocketChannel->priorityQueues[compressedmb1->priority].priorityQueue), compressedmb1);
						}
						...
						if (rsslSocketChannel->outCompression == RIPC_COMP_LZ4) /* we should not be here with LZ4 unless it was over the comp threshold && (tempLen >= sess->upperCompressionThreshold) */
						{
						...
						else
						{
							...
							compBuf.avail_out = (compressedmb2->maxLength - headerLength - footer_size);
			...
			if ((forceFlush == 0) || (forceFlush == 2))
			{
				...
				lastmb = (rtr_msgb_t*)rtr_qtool_last(&rsslSocketChannel->priorityQueues[(*msgb)->priority].priorityQueue);
				...
				if (lastmb && ((lastmb->buffer + lastmb->length) == (*msgb)->buffer))
				{
				...
				else
				{
					...
					rtr_qtool_insqt(&(rsslSocketChannel->priorityQueues[(*msgb)->priority].priorityQueue), (*msgb));
					rsslSocketChannel->bytesOutLastMsg += (RsslUInt32)((*msgb)->length);
				}
			}
...
RsslRet ipcFlushSession(RsslSocketChannel *rsslSocketChannel, RsslError *error)
{
	...
	char				iovPriority[RIPC_MAXIOVLEN + 1];
	...
	if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1003 ipcFlushSession() failed due to session shutting down.\n",
			__FILE__, __LINE__);
	...
	if (rsslSocketChannel->transportFuncs->writeVTransport == 0) {
		while (cont)
		{
			...
			/* while we have a buffer, or we dont have a buffer and have gone through
			the entire flush strategy we want to exit the while */

			/* this should get a msgb from the output queue that should be flushed */
			while (!curmsgb)
			{
				curmsgb = (rtr_msgb_t*)rtr_qtool_first(&rsslSocketChannel->priorityQueues[rsslSocketChannel->flushStrategy[rsslSocketChannel->currentOutList]].priorityQueue);
			...
			we dont have a buffer and we have gone through the strategy */
			/* since we dont require having 'L' in the strategy, we shoudl check
			if we went through the entire strategy - if we did and we dont have
			a buffer, try to grab a buffer from the low queue
			*/
			if (!curmsgb)
			{
				/* this needs to change if we add more than the high, med, and low queues */
				curmsgb = (rtr_msgb_t*)rtr_qtool_first(&rsslSocketChannel->priorityQueues[2].priorityQueue);
			}
			else
			{
			...
			if (!curmsgb)
			{
				/* if we still dont have a buffer here, we have nothing to write */
			...
			while (curmsgb)
			{
				...
				if (rsslSocketChannel->httpHeaders)
					cc = (*(rsslSocketChannel->transportFuncs->writeTransport))(rsslSocketChannel->tunnelStreamFd, curmsgb->local, lenToWrite, rwflags, error);
				else
				...
				if (cc < 0)
				{
					...
					if (rsslSocketChannel->newTunnelStreamFd && rsslSocketChannel->httpHeaders)
					{
						rsslSocketChannel->tunnelStreamFd = rsslSocketChannel->newTunnelStreamFd;
						rsslSocketChannel->newTunnelStreamFd = 0;
				...
				if (cc == lenToWrite) /* check if we wrote out everything we expected to */
				{
					...
					/* and free it */
					curmsgb = (rtr_msgb_t*)rtr_qtool_takeqh(&rsslSocketChannel->priorityQueues[curmsgb->priority].priorityQueue);
					RIPC_ASSERT(curmsgb);
	...
	while (cont)
	{
		...
		{
			curmsgb = (rtr_msgb_t*)rtr_qtool_first(&rsslSocketChannel->priorityQueues[rsslSocketChannel->nextOutBuf].priorityQueue);
			if (curmsgb) {
		...
		while ((curmsgb) && (wrtveclen < iovLength))
		{
			...
			{
				/* since the length is 0, we dont know if we will replace
				this in the write vector so we should set it back to -1 */
				iovPriority[wrtveclen] = -1;

				/* here we were getting the next buffer, then releasing the current buffer and then setting current buffer to next buffer */
				/* now, we will just release current buffer, and then after this if/else, go and get the next buffer based on priority */
				/*rtr_msgb_t *nmsgb = (rtr_msgb_t*)rtr_qtool_next(&(sess->pendOutList),curmsgb); */
				...
				rtr_qtool_remqe(&rsslSocketChannel->priorityQueues[curmsgb->priority].priorityQueue, curmsgb);
			...
			while ((!curmsgb) && (wrtveclen < iovLength))
			{
				if (rsslSocketChannel->priorityQueues[rsslSocketChannel->flushStrategy[rsslSocketChannel->currentOutList]].tempIndex > -1)
					curmsgb = (rtr_msgb_t*)rtr_qtool_next(&rsslSocketChannel->priorityQueues[rsslSocketChannel->flushStrategy[rsslSocketChannel->currentOutList]].priorityQueue, (rsslSocketChannel->priorityQueues[rsslSocketChannel->flushStrategy[rsslSocketChannel->currentOutList]].tempList[rsslSocketChannel->priorityQueues[rsslSocketChannel->flushStrategy[rsslSocketChannel->currentOutList]].tempIndex]));
				else
					curmsgb = (rtr_msgb_t*)rtr_qtool_first(&rsslSocketChannel->priorityQueues[rsslSocketChannel->flushStrategy[rsslSocketChannel->currentOutList]].priorityQueue);
			...
			if ((!curmsgb) && (wrtveclen < iovLength))
			{
				...
				if (rsslSocketChannel->priorityQueues[2].tempIndex > -1)
					curmsgb = (rtr_msgb_t*)rtr_qtool_next(&rsslSocketChannel->priorityQueues[2].priorityQueue, rsslSocketChannel->priorityQueues[2].tempList[rsslSocketChannel->priorityQueues[2].tempIndex]);
				else
					curmsgb = (rtr_msgb_t*)rtr_qtool_first(&rsslSocketChannel->priorityQueues[2].priorityQueue);
		...
		{
			if (rsslSocketChannel->httpHeaders)
				cc = (*(rsslSocketChannel->transportFuncs->writeVTransport))(rsslSocketChannel->tunnelStreamFd, wrtvec, wrtveclen, lenToWrite, rwflags, error);
			else
			...
			if (cc == lenToWrite)
			{
				...
				{
					wrtveclen--;
					curmsgb = (rtr_msgb_t*)rtr_qtool_takeqh(&rsslSocketChannel->priorityQueues[iovPriority[wrtveclen]].priorityQueue);
				...
				/* need to switch stream IDs after sending the end of chunk message */
				if (reducedIovLen && rsslSocketChannel->newTunnelStreamFd && rsslSocketChannel->httpHeaders  && rsslSocketChannel->sentControlAck)
				{
					RsslInt32 outLen;
					RsslInt32 writtenLen = 0;
					char outBuf[255];
			...
			else
			{
				...
				while (cc > 0)
				{
					...
					{
						curmsgb = (rtr_msgb_t*)rtr_qtool_takeqh(&rsslSocketChannel->priorityQueues[iovPriority[curpos]].priorityQueue);
					...
					{
						/* dont want to change output lengths here - this will be done
						when we write the entire buffer */
						curmsgb = (rtr_msgb_t*)rtr_qtool_first(&rsslSocketChannel->priorityQueues[iovPriority[curpos]].priorityQueue);
...
ipcSessInit ipcIntSessInit(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error)
{
	RsslInt32	cont = 1;
	RsslInt32	ret;
...
static ipcSessInit ipcInitTransport(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inProg, RsslError *error)
{
	RsslInt32 cc;
	...
	if (cc < 0)
	{
		//		ripcSessFail(usess,error,RIPC_SYS_ERR,0);
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
...
static ipcSessInit ipcInitClientTransport(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inProg, RsslError *error)
{
	RsslInt32 cc;
	...
	if (cc < 0)
	{
		...
		else
		{
			...
			if (rsslSocketChannel->sslProtocolBitmap == RIPC_PROTO_SSL_NONE)
			{
				_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
				snprintf(error->text, MAX_RSSL_ERROR_TEXT,
					"<%s:%d> Error: 0012 Server does not support configured TLS protocols.\n",
					__FILE__, __LINE__);
		...
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d>  Error: 0012 Invalid connection type.\n",
			__FILE__, __LINE__);
...
static ipcSessInit ipcReadHdr(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inProg, RsslError *error)
{
	RsslInt32 cc;
	ripcRWFlags				rwflags = RIPC_RW_NONE;
	...
	if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1003 ipcReadHdr() failed due to session shutting down.\n",
			__FILE__, __LINE__);
...
ipcSessInit ipcProcessHdr(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inProg, RsslError *error, RsslInt32 cc)
{
		...
		{
			{
				...
				* connection versions that dont use it add 0 to counts */
				...
				*/
				...
				case CONN_VERSION_10:
				{
					if ((compbitmapsize > 0) && (rsslSocketChannel->srvrcomp))
					{
						...
						for (i = 0; i <= RIPC_COMP_MAX_TYPE; i++)
						{
							RsslInt16 idx = ripccompressions[i][RIPC_COMP_BYTEINDEX];
							if ((idx < RIPC_COMP_BITMAP_SIZE) && (idx < (RsslInt16)compbitmapsize))
							{
								if (compbitmap[idx] & ripccompressions[i][RIPC_COMP_BYTEBIT])
								{
									rsslSocketChannel->outCompression = ripccompressions[i][RIPC_COMP_TYPE];
									break;
								}
							}
						}
						if (rsslSocketChannel->outCompression > RIPC_COMP_MAX_TYPE)
							rsslSocketChannel->outCompression = RIPC_COMP_NONE;
					...
					if (rsslSocketChannel->curInputBuf == 0)
					{
						_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
						snprintf(error->text, MAX_RSSL_ERROR_TEXT,
							"<%s:%d> Error: 1001 Could not allocate memory for session's input buffer.\n",
							__FILE__, __LINE__);

						return(RIPC_CONN_ERROR);
					}
					if (rtrUnlikely(conndebug))
						printf("session->outCompression = %d\n", rsslSocketChannel->outCompression);
				}
				break;
					...
					if (((compbitmapsize > 0) && (rsslSocketChannel->srvrcomp)) || (rsslSocketChannel->forcecomp))
					{
						...
						if (compbitmapsize > 0)
						{
							...
							for (i = 0; i <= RIPC_COMP_MAX_TYPE; i++)
							{
								...
								if ((idx < RIPC_COMP_BITMAP_SIZE) && (idx < (RsslInt16)compbitmapsize))
								{
									if ((compbitmap[idx] & ripccompressions[i][RIPC_COMP_BYTEBIT]) != 0 &&
										((ripccompressions[i][RIPC_COMP_TYPE] & rsslSocketChannel->srvrcomp) != 0))
									{
										rsslSocketChannel->outCompression = ripccompressions[i][RIPC_COMP_TYPE];
										break;
									}
						...
						else
						{
							...
							rsslSocketChannel->outCompression = RIPC_COMP_ZLIB;
						}
						if (rsslSocketChannel->outCompression > RIPC_COMP_MAX_TYPE)
							rsslSocketChannel->outCompression = RIPC_COMP_NONE;
				...
				case CONN_VERSION_12:
				{
					if (((compbitmapsize > 0) && (rsslSocketChannel->srvrcomp)) || (rsslSocketChannel->forcecomp))
					{
						...
						* so we need to make sure we dont exceed the buffer.
						...
						*/
						if (rsslSocketChannel->outCompression == RIPC_COMP_LZ4)
						{
				...
				case CONN_VERSION_14:
				{
					if (rsslSocketChannel->srvrcomp)
					{
						...
						{
							rsslSocketChannel->outCompression = (ripcCompressType)rsslSocketChannel->srvrcomp;
						}
						else if (compbitmapsize > 0)
						{
							...
							for (i = 0; i <= RIPC_COMP_MAX_TYPE; i++)
							{
								...
								if ((compbitmap[idx] & ripccompressions[i][RIPC_COMP_BYTEBIT]) != 0 &&
									((ripccompressions[i][RIPC_COMP_TYPE] & rsslSocketChannel->srvrcomp) != 0))
								{
									rsslSocketChannel->outCompression = (ripcCompressType)ripccompressions[i][RIPC_COMP_TYPE];
									break;
								}
							}
						}
				...
				/* set external ping flags for server side*/
//				sckt->pingTimeout = rsslSocketChannel->pingTimeout;
//				sckt->rsslFlags = rsslSocketChannel->rsslFlags;
...
//				sckt->protocolType = rsslSocketChannel->protocolType;
//				sckt->majorVersion = rsslSocketChannel->majorVersion;
//				sckt->minorVersion = rsslSocketChannel->minorVersion;
	...
	printf("LOCK ripcMutex -- ipcProcessHdr\n");
				...
				for (oldSession = (RsslSocketChannel*)rtr_qtool_first(&activeSessionList);
					oldSession != 0;
					oldSession = (RsslSocketChannel*)rtr_qtool_next(&activeSessionList, oldSession))
				{
	...
	printf("UNLOCK ripcMutex -- ipcProcessHdr\n");
				...
				if (oldSession->tunnelingState == RIPC_TUNNEL_INIT)
				{
					...
					rsslSocketChannel->tunnelStreamFd = oldSession->tunnelStreamFd;
					/* flip the sessionIDs */
					...
					rtr_pipe_write(&oldSession->sessPipe, "1", 1);

					if (!(rtr_pipe_create(&rsslSocketChannel->sessPipe)))
					{
						/* some error here */
						_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
						snprintf(error->text, MAX_RSSL_ERROR_TEXT,
					...
					/* if we dont have more bytes to read, we may not have gotten it yet, or this may be a reconnection */
					if (headerCursor < cc)
				...
				else
				{
					...
					if ((opCode & RIPC_TUNNEL_RECONNECT) && (oldSession->tunnelingState == RIPC_TUNNEL_ACTIVE))
					{
						...
						if (oldSession->newTunnelStreamFd)
						{
							...
							retval = (*(oldSession->transportFuncs->writeTransport))(oldSession->newTunnelStreamFd, outBuf, outLen, rwflags, error);
	...
	printf("UNLOCK ripcMutex -- ipcProcessHdr\n");
				...
				{
					/* 1st */
					oldSession->newTunnelStreamFd = (void*)(intptr_t)rsslSocketChannel->stream;
	...
	printf("UNLOCK ripcMutex -- ipcProcessHdr\n");
				...
				if ((opCode & RIPC_TUNNEL_RECONNECT) && (oldSession->tunnelingState == RIPC_TUNNEL_ACTIVE))
				{
					...
					/* send out the chunked header first if we need it - if we dont need it
					...
					if (rsslSocketChannel->httpHeaders)
					{
						...
						retval = (*(rsslSocketChannel->transportFuncs->writeTransport))(((void*)(intptr_t)rsslSocketChannel->stream), outBuf, outLen, rwflags, error);
					}
		...
		else
		{
			if (opCode == RIPC_JAVA_WITH_HTTP_TUNNELING)
			{
				...
				rsslSocketChannel->tunnelStreamFd = (void*)(intptr_t)rsslSocketChannel->stream;
				...
				if (rsslSocketChannel->httpHeaders)
					retval = (*(rsslSocketChannel->transportFuncs->writeTransport))(rsslSocketChannel->tunnelStreamFd, outBuf, outLen, rwflags, error);
				else
					retval = (*(rsslSocketChannel->transportFuncs->writeTransport))(rsslSocketChannel->transportInfo, outBuf, outLen, rwflags, error);
			...
			if (opCode & RIPC_WININET_TUNNELING)
			{
				...
				rsslSocketChannel->stream = rtr_pipe_get_read_fd(&rsslSocketChannel->sessPipe);
	...
	*/
	...
	if (rsslSocketChannel->guarBufPool == 0)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1001 Could not allocate memory for session's output buffer pool.\n",
			__FILE__, __LINE__);
	...
	{
		if (rtrUnlikely(conndebug))
			printf("\nipcProcessHdr: NOT doing renegotiation %d\n", rsslSocketChannel->stream);
...
static ipcSessInit ipcRejectSession(RsslSocketChannel *rsslSocketChannel, RsslUInt16 err, RsslError *error)
{
	...
	if (rsslSocketChannel->httpHeaders)
		retval = (*(rsslSocketChannel->transportFuncs->writeTransport))(rsslSocketChannel->tunnelStreamFd, (char*)conMsg, ipcMsgLength, rwflags, error);
	else
	...
	if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1003 ipcRejectSession() failed due to session shutting down.\n",
			__FILE__, __LINE__);
	...
	snprintf(error->text, MAX_RSSL_ERROR_TEXT,
		"<%s:%d> ipcRejectSession() Error: 1006 Connection refused.\n",
		__FILE__, __LINE__);
	...
	if (rsslSocketChannel->newStream != RIPC_INVALID_SOCKET)
	{
		if (!(rsslSocketChannel->workState & RIPC_INT_SOCK_CLOSED))
		{
			...
			/* if we dont change the state, they shoudl get a notification and this will cause them
...
static ipcSessInit ipcFinishSess(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error)
{
	char			conMsg[1024];
	RsslRet			retval;
	...
	if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1003 ipcFinishSession() failed due to session shutting down.\n",
			__FILE__, __LINE__);
	...
	* keep the state progressing.  A new internal state is needed so the next call to InitChannel goes into that method */
	...
	else
	{
		...
		if (rsslSocketChannel->lowerCompressionThreshold == 0)
		{
			...
			{
			case RIPC_COMP_ZLIB:
				rsslSocketChannel->lowerCompressionThreshold = RIPC_COMP_DFLT_THRESHOLD_ZLIB;
				break;
			case RIPC_COMP_LZ4:
				rsslSocketChannel->lowerCompressionThreshold = RIPC_COMP_DFLT_THRESHOLD_LZ4;
				break;
			default:
				break;
			}
...
static ipcSessInit ipcSendClientKey(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error)
{
	ripcRWFlags		rwflags = RIPC_RW_WAITALL;
	...
	//	if (sess->blocking == 0)
	{
		...
		if (rsslSocketChannel->version->connVersion > CONN_VERSION_13)
		{
			...
			if (rsslSocketChannel->encryptionType == TR_SL_1 && rsslSocketChannel->P != 0)
			{
				...
				/* Now expose through so UPA upper level can get this */
				rsslSocketChannel->shared_key = rsslSocketChannel->shared_key;
			}
			else
			{
				/* we dont know the servers type, so send back a 0 length key */
		...
		if (rtrUnlikely(conndebug))
			printf("ipcSendClientKey: fd %d header %d \n", rsslSocketChannel->stream, len);
	...
	printf("UNLOCK rsslSocketChannel -- ipcSendClientKey (before transportFuncs->writeTransport)\n");
		...
		/* dont need to worry about httpHeader here because this is client side only  - function pointers will take care of it */
	...
	printf("LOCK rsslSocketChannel -- ipcSendClientKey (after transportFuncs->writeTransport)\n");
		...
		if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
		{
			_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> Error: 1003 ipcSendClientKey() failed due to session shutting down.\n",
				__FILE__, __LINE__);
		...
		if (bytesSent < 0)
		{
			_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> Error: 1002 ripcWrite() failed to write version number. System errno: (%d)\n",
				__FILE__, __LINE__, errno);
...
static ipcSessInit ipcWaitClientKey(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error)
{
	RsslInt32			cc;
	ripcRWFlags			rwflags = RIPC_RW_NONE;
	char 				*hdrStart;
	RsslUInt16			length = 0;
	RsslUInt8 			keyExchange = 0;
	RsslInt32			totalMsgLength = 0;
	RsslUInt8			keyLen = 0;
...
#ifdef IPC_DEBUG
	if (rtrUnlikely(conndebug))
		printf("\nipcWaitClientKey: read header %d\n", rsslSocketChannel->stream);
#endif
	...
	* another round of IO Notification, so the app will know to call rsslRead */
	...
	if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1003 ipcWaitClientKey() failed due to session shutting down.\n",
			__FILE__, __LINE__);
...
#ifdef IPC_DEBUG
	if (rtrUnlikely(conndebug))
		printf("\ipcWaitClientKey: read header %d %d\n", rsslSocketChannel->stream, cc);
#endif
	...
	_move_u16_swap(&length, hdrStart);
	rsslSocketChannel->inputBufCursor += 2;
	rsslSocketChannel->inputBufCursor += rwfGet8(keyExchange, (hdrStart + rsslSocketChannel->inputBufCursor));
...
static ipcSessInit ipcProxyConnecting(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error)
{
	...
	if (rsslSocketChannel->blocking == 0)
	{
		...
		if (retval != 1)
		{
			if (retval != 0)
			{
				_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
				snprintf(error->text, MAX_RSSL_ERROR_TEXT,
					"<%s:%d> Error: 1002  ipcConnected() Client connect() failed.  System errno: (%d)\n",
					__FILE__, __LINE__, errno);
	...
	len += snprintf(buf + len, (1024 - len), "CONNECT %s:%s HTTP/1.0\r\n", rsslSocketChannel->hostName, rsslSocketChannel->serverName);
	len += snprintf(buf + len, (1024 - len), "User-agent: Mozilla/4.0\r\n");
	len += snprintf(buf + len, (1024 - len), "Pragma: no-cache\r\n");
	len += snprintf(buf + len, (1024 - len), "Proxy-Connection: Keep-Alive\r\n\r\n");

	if (rtrUnlikely(conndebug))
		printf("ipcProxyConnecting: fd %d header %d \n", rsslSocketChannel->stream, len);

	if (rsslSocketChannel->dbgFlags & RSSL_DEBUG_IPC_DUMP_INIT)
	{
		(*(ripcDumpOutFunc))(__FUNCTION__, buf, len, rsslSocketChannel->stream);
	}

#ifdef MUTEX_DEBUG
	printf("UNLOCK rsslSocketChannel -- ipcProxyConnecting (before transportFuncs->writeTransport)\n");
#endif
	IPC_MUTEX_UNLOCK(rsslSocketChannel);
...
#if (defined(_WINDOWS) || defined(_WIN32))
	{
		...
		for (i = 0; i<10; i++)
		{
			/* Write to the stream directly here, since it should be a TCP connection */
			bytesSent = (*(rsslSocketChannel->transportFuncs->writeTransport))(((void*)(intptr_t)(rsslSocketChannel->stream)), buf, len, rwflags, error);
			if (bytesSent > 0)
				break;
		}
	}
#else
	/* Write to the stream directly here, since it should be a TCP connection */
	bytesSent = (*(rsslSocketChannel->transportFuncs->writeTransport))(((void*)(intptr_t)rsslSocketChannel->stream),
		buf, len, rwflags, error);
#endif

#ifdef MUTEX_DEBUG
	printf("LOCK rsslSocketChannel -- ipcProxyConnecting (after transportFuncs->writeTransport)\n");
#endif
	IPC_MUTEX_LOCK(rsslSocketChannel);
	...
	if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1003 ipcProxyConnecting() failed due to session shutting down.\n",
			__FILE__, __LINE__);
	...
	if (bytesSent < 0)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1002 ipcWrite() failed to write proxy connection request. System errno: (%d)\n",
			__FILE__, __LINE__, errno);
...
static ipcSessInit ipcConnecting(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error)
{
	...
	if (rsslSocketChannel->blocking == 0)
	{
		...
		if (retval != 1)
		{
			if (retval != 0)
			{
				_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
				snprintf(error->text, MAX_RSSL_ERROR_TEXT,
					"<%s:%d> ipcConnecting() Error: 1002 client connect() failed.  System errno: (%d)\n",
					__FILE__, __LINE__, errno);
...

	{
		...
		RsslUInt16	len = 0;
		RsslUInt8	flags = 0x00;
		RsslUInt8	Bitmap = 0x00;
		...
		*/
		...
		case CONN_VERSION_14:
		{
			...
			{
#ifdef _WIN32
				//TODO add getsockname to _ENCRYPTED and HTTP transports so the if condition can be removed
			...
			{
				ripcHeaderSize = V10_MIN_CONN_HDR + RIPC_COMP_BITMAP_SIZE +
					hostnameLen + addrLen + 2;
				if (rsslSocketChannel->version->connVersion > CONN_VERSION_11)
					++ripcHeaderSize;
				ripcHead[8] = (char)ripcHeaderSize;
				ripcHead[9] = RIPC_COMP_BITMAP_SIZE;
				MemCopyByInt((ripcHead + 10), (char*)rsslSocketChannel->compressionBitmap, RIPC_COMP_BITMAP_SIZE);
				/* set len to the next element for the variable portion */
				len = 10 + RIPC_COMP_BITMAP_SIZE;
			}
		...
		if (rtrUnlikely(conndebug))
			printf("ripcConnecting: fd %d header %d \n", rsslSocketChannel->stream, headerSize);
	...
	printf("LOCK rsslSocketChannel -- ipcConnecting (after transportFuncs->writeTransport)\n");
		...
		if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
		{
			_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> Error: 1003 ipcConnecting() failed due to session shutting down.\n",
				__FILE__, __LINE__);
...
static ipcSessInit ipcClientAccept(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error)
{
	...
	fdtemp = accept(rsslSocketChannel->stream, (struct sockaddr *)0, (RsslInt32 *)0);
	...
	if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1003 ipcClientConnect() failed due to session shutting down.\n",
			__FILE__, __LINE__);
...
//	inPr->newSocket = sckt;
//	inPr->newSocket = rsslSocketChannel->newStream;
...
static ipcSessInit ipcReconnectOld(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error)
{
	void*				userSpecPtr = 0;
	RsslInt32			csFlags = RIPC_INT_CS_FLAG_NONE;
...
static ipcSessInit ipcReconnectSocket(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error)
{
	...
	if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1003 ipcReconnectOld() failed due to session shutting down.\n",
			__FILE__, __LINE__);
	...
	if (rsslSocketChannel->connType == RSSL_CONN_TYPE_ENCRYPTED)
	{
		...
		{
			/* Directly connecting without HTTP proxy, so SSL/TLS connection should start as soon as the TCP channel has been established */
	...
	if (rsslSocketChannel->connType == RSSL_CONN_TYPE_EXT_LINE_SOCKET)
	{
		...
		rsslSocketChannel->transportFuncs = &(transFuncs[RIPC_EXT_LINE_SOCKET_TRANSPORT]);
	}
	...
	if ((sock_fd = (*(rsslSocketChannel->transportFuncs->connectSocket))(
		&portnum, rsslSocketChannel, csFlags, &userSpecPtr, error)) <= 0)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
...
//	inPr->newSocket = rsslSocketChannel->stream;
	_rsslSocketChannelToIpcSocket(&(inPr->newSocket), rsslSocketChannel);
...
ipcSessInit ipcWaitProxyAck(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error)
{
	char			buf[MAX_RSSL_ERROR_TEXT + V10_MIN_CONN_HDR + 1];
	RsslInt32		cc;
	RsslInt32		isOk = 1;
	RsslInt32		eol = 0;
	RsslInt32		msgLen = 0;
	...
	cc = (*(rsslSocketChannel->transportFuncs->readTransport))(rsslSocketChannel->transportInfo,
		buf, (MAX_RSSL_ERROR_TEXT + V10_MIN_CONN_HDR + 1), rwflags, error);

#ifdef MUTEX_DEBUG
	printf("LOCK rsslSocketChannel -- ipcWaitProxyAck\n");
#endif
	IPC_MUTEX_LOCK(rsslSocketChannel);
	...
	if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1003 ipcWaitProxyAck() failed due to session shutting down.\n",
			__FILE__, __LINE__);
	...
	if (cc < 0)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1002 Could not read Proxy HTTP Ack.  System errno: (%d)\n",
			__FILE__, __LINE__, errno, buf);

		return(RIPC_CONN_ERROR);
	}
	else if (cc == 0)
	{
		/* This will happen when there is no data atall to be read, and the read fails with error _IPC_WOULD_BLOCK or EINTR */
		return (RIPC_CONN_IN_PROGRESS);
	}

	/* Check for 200 OK */
	if ((msgLen = ipcGetHttpAck(buf, cc)) == 0)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1002 Could not read Proxy HTTP Ack received. System errno: (%d)\nText: %s\n",
			__FILE__, __LINE__, errno, buf);
	...
	* that send data as soon as the session becomes active.
	*/
	if (cc == msgLen)
	{
		/* this should really only happen for OpenSSL - we should restrict proxy stuff to openssl */
		if (rsslSocketChannel->connType == RSSL_CONN_TYPE_ENCRYPTED)
		{
			if (getSSLProtocolTransFuncs(rsslSocketChannel, rsslSocketChannel->sslProtocolBitmap) != 0)
			{
		...
		rsslSocketChannel->transportInfo = (*(rsslSocketChannel->transportFuncs->newClientConnection))(
			rsslSocketChannel->stream, &initcomplete, 0, error);
		...
		if (rsslSocketChannel->transportInfo == 0)
		{
			...
			_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> Error: 1008 Transport functions have not been properly set.\n",
				__FILE__, __LINE__);
		...
		/* Technically, this should happen for any proxies used... */
		if ((!rsslSocketChannel->connType) || ((rsslSocketChannel->connType == RSSL_CONN_TYPE_ENCRYPTED) && initcomplete))
		{
			/* normal ripc connection - send connect request */
			rsslSocketChannel->intState = RIPC_INT_ST_CONNECTING;
		}

		return ipcIntSessInit(rsslSocketChannel, inPr, error);
	}
	else
	{
		/* we should not get more bytes than this */
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1002 Could not read Proxy HTTP Ack received. System errno: (%d)\n",
			__FILE__, __LINE__, errno);
...
ipcSessInit ipcWaitAck(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error, char* curBuf, RsslInt32 bufLen)
{
	char			buf[1024];
	RsslInt32		cc = 0;
	RsslInt32		idOffset = 0; /* will always be 0 if not tunneling */
	RsslInt32		opCode = 0;
	RsslUInt16		numUserMsg = 0;
	...
	if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1003 ipcWaitAck() failed due to session shutting down.\n",
			__FILE__, __LINE__);
	...
	if (rtrUnlikely(conndebug))
		printf("ipcWaitAck: fd %d cc %d err %d\n", rsslSocketChannel->stream, cc, ((cc <= 0) ? errno : 0));
	...
	/* future versions will probably make use of the version number sent back in the ACK message. */
	{
		...
		if ((rsslSocketChannel->keyExchange == 1) && (versionNumber > RIPC_VERSION_13))
		{
			...
			*/
			...
			tempIter += rwfGet8(keyExchange, (buf + tempIter));
			...
			* if we see soemthing else, skip it */
			...
			else
			{
				...
				/* we dont know how to handle other types in this connection version */
				tempIter += encLen;
	...
	for (i = 0; i < RIPC_MAX_PRIORITY_QUEUE; i++);
	{
		rtr_qtool_init(&rsslSocketChannel->priorityQueues[i].priorityQueue, 0);
...
RsslRet rsslSocketBind(rsslServerImpl* rsslSrvrImpl, RsslBindOptions *opts, RsslError *error )
{
	...
	if (opts->numInputBuffers <= 1)
	{
		...
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> ripcSBind() Error: 1004 invalid number of input buffers specified(<%d>), must be at least <%d>.\n",
	...
	if (opts->guaranteedOutputBuffers < 1)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, 0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> ripcSBind() Error: 1004 invalid number of guaranteed output buffers <%d>, must be at least <%d>.\n",
			__FILE__, __LINE__, opts->guaranteedOutputBuffers, 1);
		return RSSL_RET_FAILURE;
	}

	if ((opts->connectionType != RSSL_CONN_TYPE_SOCKET) && (opts->connectionType != RSSL_CONN_TYPE_HTTP) && (opts->connectionType != RSSL_CONN_TYPE_EXT_LINE_SOCKET))
	{
		_rsslSetError(error, NULL, RSSL_RET_INVALID_ARGUMENT, 0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> rsslSocketBind() Error: 0006 Connection type %d is not supported.\n", __FILE__, __LINE__, opts->connectionType);
		return RSSL_RET_FAILURE;
	}

	if (ripcValidServerName(opts->serviceName, MAX_SERV_NAME) < 0)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, 0);
	...
	if ((opts->compressionType & ~RIPC_COMP_ALL_TYPE) != 0)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, 0);
	...
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, 0);
		snprintf(error->text, MAX_RIPC_ERROR_TEXT, "<%s:%d> Error: 1004 SSL Tunneling not supported in this library.", __FILE__, __LINE__);
		return RSSL_RET_FAILURE;
	}
	...
	if(opts->compressionType && opts->forceCompression)
	{
		...
		if (count == 0)
		{
			...
			freeRsslServerSocketChannel(rsslServerSocketChannel);
			return RSSL_RET_FAILURE;
		}
		if (count > 1)
		{
	...
	rsslServerSocketChannel = (RsslServerSocketChannel*)_rsslMalloc(sizeof(RsslServerSocketChannel));

	if (rsslServerSocketChannel == 0)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, 0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> rsslSocketBind() Error: 0005 Could not allocate memory for binding socket.\n", __FILE__, __LINE__, opts->connectionType);
		return RSSL_RET_FAILURE;
	}
	...
	if ((rsslServerSocketChannel->maxNumMsgs - opts->guaranteedOutputBuffers) > poolSize)
	{
		...
		_rsslSetError(error, NULL, RSSL_RET_INVALID_ARGUMENT, 0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> rsslSocketBind() Error: 0012 SharedPoolSize (%d) is less than configured number of shared pool buffers allowed per connection (%d).\n", __FILE__, __LINE__, poolSize, (rsslServerSocketChannel->maxNumMsgs - opts->guaranteedOutputBuffers));
	...
	rsslServerSocketChannel->connType = opts->connectionType;
...
#endif
		RTR_MUTEX_INIT(rsslSrvrImpl->sharedBufPoolMutex, mterr);	
		rsslSrvrImpl->hasSharedBufPool = RSSL_TRUE;
		serverPool = ripcCreatePool(poolSize, &(rsslSrvrImpl->sharedBufPoolMutex));
	}
	else
	{
		rsslSrvrImpl->hasSharedBufPool = RSSL_FALSE;
		serverPool = ripcCreatePool(poolSize, 0);
	}

	if (serverPool == NULL)
	{
		_rsslSetError(error, NULL, RSSL_RET_BUFFER_NO_BUFFERS,  0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> rsslBind() Error: 0005 Unable to create buffer pool.\n", __FILE__, __LINE__);
		freeRsslServerSocketChannel(rsslServerSocketChannel);
		return RSSL_RET_FAILURE;
	}
	else
	...
	{
		if (rsslLoadInitTransport(&(transFuncs[RIPC_EXT_LINE_SOCKET_TRANSPORT]),
											0,
	...
	//rtr_qtool_insqt(&(user_srvr->server.userList), user_srvr);
	...
	rsslSrvrImpl->Server.portNumber = net2host_u16(ripcGetServByName(rsslServerSocketChannel->serverName));
...
RsslRet rsslSocketConnect(rsslChannelImpl* rsslChnlImpl, RsslConnectOptions *opts, RsslError *error)
{
	...
	{
		_rsslSetError(error, NULL, RSSL_RET_INVALID_ARGUMENT, 0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> rsslConnect() Error: 0013 No hostName or unified.address provided.\n", __FILE__, __LINE__);
		return RSSL_RET_FAILURE;
	}
	...
	{
		_rsslSetError(error, NULL, RSSL_RET_INVALID_ARGUMENT, 0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> rsslConnect() Error: 0013 No serviceName or unified.serviceName provided.\n", __FILE__, __LINE__);
		return RSSL_RET_FAILURE;
	}
	...
	rsslSocketChannel = (RsslSocketChannel*)_rsslMalloc(sizeof(RsslSocketChannel));

	if (rsslSocketChannel == 0)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, 0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> rsslConnect() Error: 0005 Failed to allocate the socket channel structure.\n", __FILE__, __LINE__);
		return RSSL_RET_FAILURE;
	}

	/* initialize rsslSocketChannel */
	...
	/* If we are on windows and using HTTP/Encrypted we shouldnt set these because options will come in from WinInet/IE settings */
	...
	if (ripcValidServerName(rsslSocketChannel->serverName,MAX_SERV_NAME) < 0)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, 0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> ipcCBind() Error: 1004 Invalid serviceName or port number specified <%s>\n",
	...
	if (rsslSocketChannel->numGuarOutputBufs < 1)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, 0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> ipcCBind() Error: 1004 invalid number of guaranteed output buffers <%d>, must be at least <%d>.\n",
	...
	case RSSL_CONN_TYPE_HTTP:
		...
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, 0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> Error: 1004 SSL Tunneling not supported in this library.\n", 
				__FILE__, __LINE__);
	...
	case RSSL_CONN_TYPE_ENCRYPTED:
		...
		rsslSocketChannel->sslProtocolBitmap &= (RIPC_PROTO_SSL_TLS_V1 | RIPC_PROTO_SSL_TLS_V1_1 | RIPC_PROTO_SSL_TLS_V1_2);
		if(rsslSocketChannel->sslProtocolBitmap == 0)
		{
			_rsslSetError(error, NULL, RSSL_RET_FAILURE, 0);
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
					"<%s:%d> Error: 0012 No valid encryption protocol set.\n",
					__FILE__, __LINE__);
	...
	{
		RsslInt16 idx = ripccompressions[rsslSocketChannel->compression][RIPC_COMP_BYTEINDEX];
		if (idx < RIPC_COMP_BITMAP_SIZE)
			rsslSocketChannel->compressionBitmap[idx] |= ripccompressions[rsslSocketChannel->compression][RIPC_COMP_BYTEBIT];
	}

	/* normal connection */
	if ((sock_fd=(*(rsslSocketChannel->transportFuncs->connectSocket))(
		&portnum, rsslChnlImpl->transportInfo, csFlags, &userSpecPtr, error)) <= 0)
	{
		ripcRelSocketChannel(rsslSocketChannel);
		return RSSL_RET_FAILURE;
	}
	...
	else
	{
		...
		if (rsslSocketChannel->transportInfo == 0)
		{
			...
			_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> rsslConnect Error: 1001 newClientConnection.\n",
				__FILE__, __LINE__);

			return RSSL_RET_FAILURE;
		}

		if (((rsslSocketChannel->connType == RSSL_CONN_TYPE_ENCRYPTED) && !initcomplete) || ((rsslSocketChannel->connType == RSSL_CONN_TYPE_HTTP)
			&& !initcomplete) || ((rsslSocketChannel->connType == RSSL_CONN_TYPE_EXT_LINE_SOCKET) && !initcomplete))
			rsslSocketChannel->intState = RIPC_INT_ST_CLIENT_TRANSPORT_INIT;
	}
//	RIPC_ASSERT(initcomplete);

	if (rsslSocketChannel->blocking)
	{
		ripcSessInProg inPr;
		ipcSessInit ret = RIPC_CONN_IN_PROGRESS;
	...
	printf("LOCK rsslSocketChannel -- rsslConnect blocking\n");
			...
			if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
			{
				_rsslSetError(error, NULL, RSSL_RET_FAILURE, 0);
				snprintf(error->text, MAX_RSSL_ERROR_TEXT,
					"<%s:%d> Error: 1003 rsslConnect failed due to channel shutting down.\n",
					__FILE__, __LINE__);
	...
	printf("UNLOCK rsslSocketChannel -- rsslConnect blocking\n");
			...
			else if (rsslSocketChannel->intState != RIPC_INT_ST_ACTIVE)
			{
				_rsslSetError(error, NULL, RSSL_RET_FAILURE, 0);
				snprintf(error->text, MAX_RSSL_ERROR_TEXT,
						"<%s:%d> Error: 1003 rsslConnect failed because the channel is not active.\n",
						__FILE__, __LINE__);
...
RsslSocketChannel *ipcNewClientChannel()
{
	RsslInt32 i = 0;
...
static RsslSocketChannel *ipcClientChannel(rsslServerImpl* serverImpl, RsslError *error)
{
	...
	if (rtrUnlikely(conndebug))
		printf("\nipcClientChannel: rssCh->stream:%d rssCh->conTyp:%d\n", 
					rsslServerSocketChannel->stream, rsslServerSocketChannel->connType);

	if ((rsslSocketChannel = ipcNewClientChannel()) == 0)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
	...
	case RSSL_CONN_TYPE_ENCRYPTED:
		...
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 0012 Encrypted connection type (%d) not supported for this platform. \n",
		...
		if (openSSLInit == 0)
		{
			/* Initialize open ssl; if we support other non windows platforms besides Linux we may need a different ifdef here */
			if (ripcInitializeSSL(transOpts.jitOpts.libsslName, transOpts.jitOpts.libcryptoName) == 1)
				openSSLInit = 1;
			else
			{
		...
		rsslSocketChannel->sslProtocolBitmap = ripcGetSupportedSSLVersion();
		if (getSSLProtocolTransFuncs(rsslSocketChannel, rsslSocketChannel->sslProtocolBitmap) != 0)
		{
			_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
		...
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 0012 SSL Tunneling not supported in this library.\n", \
			__FILE__, __LINE__);
	...
	/*dont need this as we do it later when processing connect msgs */
	/*session->curInputBuf = rtr_smplcDupMsg(gblInputBufs,session->inputBuffer);  */

	if (rsslSocketChannel->inputBuffer == 0) /*|| (session->curInputBuf == 0)) */
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1001 Could not allocate memory for session's input buffer.\n",
	...
	//rtr_qtool_insqt(&(session->userList), nusess);

	/* Insert the new RsslSocketChannel into the server list */
	if (multiThread)
		RTR_MUTEX_LOCK(ripcMutex, mterr);
	rtr_qtool_insqt(&activeSessionList, rsslSocketChannel);
	if (multiThread)
		RTR_MUTEX_UNLOCK(ripcMutex, mterr);
...
rsslChannelImpl* rsslSocketAccept(rsslServerImpl *rsslSrvrImpl, RsslAcceptOptions *opts, RsslError *error)
{
	...
	if ((rsslChnlImpl = _rsslNewChannel()) == 0)
	{
		...
		snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> rsslAccept() Error: 0005 Could not allocate memory for new channel\n", __FILE__, __LINE__);
		return 0;
	}	
...
RsslRet rsslSocketCloseChannel(rsslChannelImpl* rsslChnlImpl, RsslError *error)
{
	...
	if ((rsslChnlImpl->returnBufferOwner == 1) && (rsslChnlImpl->returnBuffer.data))
	{
		...
		retVal = ipcShutdownSession(rsslSocketChannel, error);

		if (retVal < RSSL_RET_SUCCESS)
		{
			/* should we return here?? */
//			_rsslErrorMapping(error, &rsslChnlImpl->Channel, &ripc_error);
			error->channel = &rsslChnlImpl->Channel;
...
RSSL_RSSL_SOCKET_IMPL_FAST(RsslBuffer*) rsslSocketRead(rsslChannelImpl* rsslChnlImpl, RsslReadOutArgs *readOutArgs, RsslRet *readRet, RsslError *error)
{
	...
	RsslUInt32     totalInBytes = 0;
	RsslUInt32     totalUncompInBytes = 0;
	RsslInt32      inBytes = 0;
...
#ifdef MUTEX_DEBUG
	printf("LOCK rsslSocketChannel -- rsslSocketRead\n");
#endif
	IPC_MUTEX_LOCK(rsslSocketChannel);
	if ((rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND) || rsslSocketChannel->intState != RIPC_INT_ST_ACTIVE)
	{
		_rsslSetError(error, &rsslChnlImpl->Channel, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1003 rsslSocketRead() failed due to channel is not active.\n",
			__FILE__, __LINE__);

		return NULL;
	}
#ifdef MUTEX_DEBUG
	printf("UNLOCK rsslSocketChannel -- rsslSocketRead\n");
#endif
	IPC_MUTEX_UNLOCK(rsslSocketChannel);
...
#endif
		if (!RTR_MUTEX_LOCK(rsslChnlImpl->chanMutex, mterr))
		{
...
#endif
			RTR_MUTEX_UNLOCK(rsslChnlImpl->chanMutex, mterr);
		}
	...
	printf("UNLOCK rsslChnlImpl->chanMutex -- rsslSocketRead\n");
				...
				snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> rsslRead() Error: 0005 Cannot allocate memory of size %d for read buffer.\n", __FILE__, __LINE__, ripcFragSize);
				*readRet = RSSL_RET_FAILURE;
				return NULL;
			}
			...
			if (rsslAssemblyBuf->readCursor == rsslAssemblyBuf->buffer.length)
			{
				...
				/* its not in the queue yet so we dont have to remove it */
			...
			else
			{
				...
				rsslTempAssemblyBuf = (rsslAssemblyBuffer*)rtr_hasht_find(&rsslChnlImpl->assemblyBuffers, rsslAssemblyBuf);
				
				if (rsslTempAssemblyBuf)
				{
					/* we found one */

					rtr_hasht_remove(&rsslChnlImpl->assemblyBuffers, rsslTempAssemblyBuf);
					if (memoryDebug)
				...
				rtr_hasht_linkInit(&(rsslChnlImpl->assemblyBuffers), rsslAssemblyBuf);
				rtr_hasht_add(&(rsslChnlImpl->assemblyBuffers), rsslAssemblyBuf, rsslAssemblyBuf);
		...
		{	/* should be another/subsequent fragment in the current message */
			...
			if (ripcBuffer->length == 0)	/* if we get a subsequent fragment with a zero size, ignore it */
			{
				/* we cant just blindly return ping here because there may be more data in the input buffer.  
				   to combat this, check if ripc said there is more data to read, we dont know whether it is a full message 
			...
			rsslAssemblyBuf = (rsslAssemblyBuffer*)rtr_hasht_find(&rsslChnlImpl->assemblyBuffers, &rsslAssemblyBufferSearch);
	...
	printf("UNLOCK rsslChnlImpl->chanMutex -- rsslSocketRead\n");
			...
			if (rsslAssemblyBuf->readCursor == rsslAssemblyBuf->buffer.length)
			{
				...
				rtr_hasht_remove(&rsslChnlImpl->assemblyBuffers, rsslAssemblyBuf);
				if (memoryDebug)
					printf("removing from assemblyBuffers hash\n");
				...
				/* clean up rsslAssemblyBuf and */
				/* release rsslAssemblyBuf to the free list */
				_rsslFree(rsslAssemblyBuf);
				rsslAssemblyBuf = 0;
			}
			else
		...
		if (ripcMoreData)
		{
			*readRet = ripcMoreData;
			...
			/* it is possible that the returnBuffer is null in this case -
			   this is because of fragmentation, if we receive multiple fragments but dont have the 
	...
	RsslWriteOutArgs *writeOutArgs, RsslError *error)
{
	...
	if ((*ripcBuffer) && (!(rsslBufImpl->fragmentationFlag)) && (rsslBufImpl->writeCursor == 0))
	{
		...
		retVal = ipcWriteSession(rsslSocketChannel, rsslBufImpl, writeFlags, &outBytes, &uncompOutBytes, (writeFlags & RSSL_WRITE_DIRECT_SOCKET_WRITE) != 0, error);
...
#endif
				RTR_MUTEX_LOCK(rsslChnlImpl->chanMutex, mterr);
			}
	...
	printf("UNLOCK rsslChnlImpl->chanMutex -- rsslSocketWrite\n");
		...
		while (tempSize > 0)
		{
			...
			if (tempSize >= (rsslChnlImpl->maxMsgSize - fragHeaderSize))
			{
			...
			(*ripcBuffer) = ipcFragmentationDataBuffer(rsslSocketChannel, 0, tempSize, error);
			if ((*ripcBuffer) == NULL)
			{
	...
	printf("LOCK rsslChnlImpl->chanMutex -- rsslSocketWrite\n");
		...
		if (rtr_qtool_headActive((&rsslChnlImpl->activeBufferList), rsslBufImpl))
		{
			rtr_qtool_remqe(&(rsslChnlImpl->activeBufferList), rsslBufImpl);
			if (memoryDebug)
	...
	printf("LOCK rsslChnlImpl->chanMutex -- rsslSocketWrite\n");
		...
		rtr_qtool_headInit(&(rsslChnlImpl->freeBufferList), rsslBufImpl);
		rtr_qtool_insqt(&(rsslChnlImpl->freeBufferList), rsslBufImpl);
		if (multiThread == RSSL_LOCK_GLOBAL_AND_CHANNEL)
		{
#ifdef MUTEX_DEBUG
	printf("UNLOCK rsslChnlImpl->chanMutex -- rsslSocketWrite\n");
...
RSSL_RSSL_SOCKET_IMPL_FAST(rsslBufferImpl*) rsslSocketGetBuffer(rsslChannelImpl *rsslChnlImpl, RsslUInt32 size, RsslBool packedBuffer, RsslError *error)
{
	...
	if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
	{
		_rsslSetError(error, &rsslChnlImpl->Channel, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1003 rsslGetBuffer() failed due to channel shutting down.\n",
			__FILE__, __LINE__);
	...
	else if (rsslSocketChannel->intState != RIPC_INT_ST_ACTIVE)
	{
		_rsslSetError(error, &rsslChnlImpl->Channel, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1003 rsslGetBuffer() failed because the channel is not active.\n",
			__FILE__, __LINE__);
	...
	else
	{
		...
		if (packedBuffer)
		{
			...
			snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> rsslGetBuffer() Error: 0015 Cannot get a packed buffer larger than maximum message size for connection.\n", __FILE__, __LINE__);
			return NULL;
		}
		...
		*/
		...
		{
			_rsslSetError(error, &rsslChnlImpl->Channel, RSSL_RET_BUFFER_NO_BUFFERS, 0);
			snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> rsslGetBuffer() Error: 0016 Cannot obtain enough buffers for fragmentation to occur.\n", __FILE__, __LINE__);
			return NULL;
		}
		else
		{
			fragmentation = 1;
			ipcBuf = ipcFragmentationDataBuffer(rsslSocketChannel, 1, size, error);
		}
	}
	...
	{
		_rsslSetError(error, &rsslChnlImpl->Channel, RSSL_RET_BUFFER_NO_BUFFERS, 0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> rsslGetBuffer() Error: 0016 Cannot allocate memory of size %d for buffer.\n", __FILE__, __LINE__, size);
...
{
	if (ipcReleaseDataBuffer(((RsslSocketChannel*)rsslChnlImpl->transportInfo), ((rtr_msgb_t*)rsslBufImpl->bufferInfo), error) < RSSL_RET_SUCCESS)
	{
		error->channel = &rsslChnlImpl->Channel;
		return RSSL_RET_FAILURE;
	}
...
RSSL_RSSL_SOCKET_IMPL_FAST(RsslBuffer*) rsslSocketPackBuffer(rsslChannelImpl *rsslChnlImpl, rsslBufferImpl *rsslBufImpl, RsslError *error)
{
	...
	else	/* There isnt enough room left to store another message (and its length) */
	{
		...
		rsslBufImpl->buffer.data = 0;		/* set this to zero. The alternative (which we dont like) is to point beyond the buffer */
	}
...
RSSL_RSSL_SOCKET_IMPL_FAST(RsslRet) rsslSocketGetChannelInfo(rsslChannelImpl *rsslChnlImpl, RsslChannelInfo *info, RsslError *error)
{
	...
	info->compressionType = (ripcCompressType)rsslSocketChannel->outCompression;
...
RSSL_RSSL_SOCKET_IMPL_FAST(RsslRet) rsslSocketGetSrvrInfo(rsslServerImpl *rsslSrvrImpl, RsslServerInfo *info, RsslError *error)
{
	...
	if (rsslServerSocketChannel == 0)
	{
		...
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
			"<%s:%d> Error: 1004 ipc GetServerInfo failed, server is NULL.\n", __FILE__, __LINE__);
	...
	else
	{
		_rsslSetError(error, (RsslChannel*)(&rsslSrvrImpl->Server), RSSL_RET_FAILURE, 0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> Error: 1004 ipcGetServerInfo() failed, no shared buffer pool.\n", __FILE__, __LINE__);
...
RSSL_RSSL_SOCKET_IMPL_FAST(RsslRet) rsslSocketSrvrIoctl(rsslServerImpl *rsslSrvrImpl, RsslIoctlCodes code, void *value, RsslError *error)
{
	...
	if (rsslServerSocketChannel == 0)
	{
		_rsslSetError(error, (RsslChannel*)(&rsslSrvrImpl->Server), RSSL_RET_FAILURE, 0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> Error: 1004 rsslSocketSrvrIoctl() failed, failed, server is NULL.\n", __FILE__, __LINE__);
	...
	switch(code)
	{
		case RSSL_SERVER_NUM_POOL_BUFFERS:
		{
			if (iValue < 1)
			{
				...
				_rsslSetError(error, (RsslChannel*)(&rsslSrvrImpl->Server), RSSL_RET_FAILURE, 0);
				snprintf(error->text, MAX_RSSL_ERROR_TEXT,
						"<%s:%d> rsslServerIoctl() Error: 0017 Invalid number of pool buffers specified (%d).\n", __FILE__, __LINE__, iValue);
	...
	printf("UNLOCK rsslServerSocketChannel -- rsslSocketSrvrIoctl\n");
			...
			if (rtr_dfltcSetMaxBufs(rsslServerSocketChannel->sharedBufPool, iValue) < 0)
			{
				_rsslSetError(error, (RsslChannel*)(&rsslSrvrImpl->Server), RSSL_RET_FAILURE, 0);
				snprintf(error->text, MAX_RSSL_ERROR_TEXT,
					"<%s:%d> Error: 1001 rsslServerIoctl() failed, could not change pool size from <%d> to <%d>\n", __FILE__, __LINE__, rsslServerSocketChannel->sharedBufPool->maxBufs, iValue);
	...
	printf("UNLOCK rsslServerSocketChannel -- rsslSocketSrvrIoctl\n");
		...
		case RSSL_SERVER_PEAK_BUF_RESET: 
		{
			if (rtr_dfltcResetPeakNumBufs(rsslServerSocketChannel->sharedBufPool) < 0)
			{
				_rsslSetError(error, (RsslChannel*)(&rsslSrvrImpl->Server), RSSL_RET_FAILURE, 0);
				snprintf(error->text, MAX_RSSL_ERROR_TEXT,
						"<%s:%d> Error: 1002 rsslServerIoctl() failed, could not reset peak number of buffers used.\n", __FILE__, __LINE__);
	...
	printf("UNLOCK rsslServerSocketChannel -- rsslSocketSrvrIoctl\n");
		...
		default:
		{
			...
			_rsslSetError(error, (RsslChannel*)(&rsslSrvrImpl->Server), RSSL_RET_FAILURE, 0);
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> rsslServerIoctl() Error: 0017 Invalid RSSL Server IOCtl code (%d).\n", __FILE__, __LINE__, code);
...
printf("UNLOCK rsslSocketChannel -- rsslSocketIoctl\n");
	...
	case RSSL_COMPRESSION_THRESHOLD:
		...
		else
		{
			_rsslSetError(error, (RsslChannel*)(&rsslChnlImpl->Channel), RSSL_RET_FAILURE, 0);
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
					"<%s:%d> Error: 1004 rsslSocketIoctl() failed, could not set the compression threshold mark to <%d>, must be greater than 30 bytes\n",
					__FILE__, __LINE__, iValue);
	...
	default:
		_rsslSetError(error, (RsslChannel*)(&rsslChnlImpl->Channel), RSSL_RET_FAILURE, 0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> Error: 1004 rsslSocketIoctl() failed, invalid ioctl code <%d>\n",
				__FILE__, __LINE__, code);
...
RsslInt32 ipcInitialize(RsslInt32 numServers, RsslInt32 numClients, RsslInitializeExOpts *initOpts, RsslError *error)
{
	...
	printf("MUTEX_INIT ripcMutex -- ipcInitialize\n");
			...
			RTR_MUTEX_INIT(ripcMutex, mterr);
			gblmutexinit = 1;
		}
	...
	if (!initialized)
	{
		//rtr_qtool_init(&freeServerList,0);
		//rtr_qtool_init(&freeSessionList, 0);
		rtr_qtool_init(&activeSessionList, 0);

		//rtr_qtool_init(&freeUserServerList, 0);
		//rtr_qtool_init(&freeUserSessionList, 0);
		...
		{
			transOpts.jitOpts.libsslName = (char*)malloc(128);
			transOpts.jitOpts.libcryptoName = (char*)malloc(128);
			...
			snprintf(transOpts.jitOpts.libsslName, sizeof(transOpts.jitOpts.libsslName), "%s", initOpts->jitOpts.libsslName);
			snprintf(transOpts.jitOpts.libcryptoName, sizeof(transOpts.jitOpts.libcryptoName), "%s", initOpts->jitOpts.libcryptoName);
		}
#endif

		protocolNumber = 6;  /* tcp (transmission control protocol) */

		if ((gblInputBufs = rtr_smplcAllocatePool(&ripcMutex)) == 0)
		{
			error->rsslErrorId = RSSL_RET_FAILURE;
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> Error: 1001 rtr_smplcAllocatePool() failed.\n",
				__FILE__, __LINE__);
		...
		if (rtr_socket_startup(eText) < 0)
		{
			error->rsslErrorId = RSSL_RET_FAILURE;
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> ipcInitialize() Error: 1001 rtr_socket_startup() failed. %s\n",
...
#if defined(SIGPIPE)
		if (RTR_IGNORE_SIG_HANDLER(SIGPIPE) == RTRFALSE)
		{
			_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> ipcInitialize() Error: 1001 Signal function call (SIGPIPE) failed (%d)",
				__FILE__, __LINE__, errno);
...
#endif
#if defined(SIGURG)
		if (RTR_IGNORE_SIG_HANDLER(SIGURG) == RTRFALSE)
		{
			_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
			snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> ipcInitialize() Error: 1001 Signal function call (SIGURG) failed (%d)",
				__FILE__, __LINE__, errno);
		...
		/* Comment code to pre-allocate Server object
		for (i = 0; i<numServers; i++)
		{
			//if ((srvr = ripcCreateServer()) != 0)
			//rtr_qtool_insqt(&freeServerList,srvr);
			if ((user_srvr = ripcCreateUserServer()) != 0)
				rtr_qtool_insqt(&freeUserServerList, user_srvr);
		}

		for (i = 0; i<numClients; i++)
		{
			if ((session = ipcCreateSess()) != 0)
				rtr_qtool_insqt(&freeSessionList, session);

			if ((usession = ipcCreateUserSess()) != 0)
				rtr_qtool_insqt(&freeUserSessionList, usession);
		}
		*/
...
RsslInt32 ipcCleanup()
{
	...
	{
		int i;
		// Remove the following code as it only cleans up the free list
		//ripcCleanSrvr();
		//ipcCleanSess();
...
#endif
				RTR_MUTEX_DESTROY(ripcMutex, mterr);
				gblmutexinit = 0;
			}
		}
	}
...
RsslInt32 ipcShutdownServer(RsslServerSocketChannel* rsslServerSocketChannel, RsslError *error)
{
	...
	if (IPC_NULL_PTR(socket, "ipcShutdownServer", "server", error))
		return RSSL_RET_FAILURE;
	...
	if (rsslServerSocketChannel != 0)
	{
		if (SSLTransFuncs.freeSSLServer)
...
{
	rtr_mutex *mutex = rsslServerSocketChannel->mutex;
...
RSSL_RSSL_SOCKET_FAST(RsslRet) rsslCloseSocketSrvr(rsslServerImpl *rsslSrvrImpl, RsslError *error) 
{
	...
	if (rsslSrvrSocketChannel != 0)
	{
		...
		{
			(*(getSSLTransFuncs()->freeSSLServer))(rsslSrvrSocketChannel->transportInfo, error);
		}
#endif
	...
	//if (rtr_qtool_headActive(&activeServerList, srvr))
	//{
	//	rtr_qtool_remqe(&activeServerList, srvr);
	//	if (memoryDebug)
	//		printf("removing from activeServerList\n");
	//}

	//rtr_qtool_headInit(&freeServerList, srvr);
	//rtr_qtool_insqt(&freeServerList, srvr);
	//if (memoryDebug)
...
RsslRet rsslSocketInitialize(RsslInitializeExOpts *initOpts, RsslError *error)
{
	...
	if (retVal < RSSL_RET_SUCCESS)
//		_rsslErrorMapping(error, NULL, &ripc_error);
		return retVal;
	else
	{
...
RsslInt32 rsslSocketGetSockOpts( RsslSocketChannel *rsslSocketChannel, RsslInt32 code, RsslInt32* value, RsslError *error)
{
	...
	if ((code < 0) || (code > RIPC_MAX_IOCTL_CODE))
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> rsslSocketGetSockOpts() failed for code %d\n",
				__FILE__, __LINE__, code);
		error->rsslErrorId = RSSL_RET_FAILURE;
	...
	if (rsslSocketChannel->workState & RIPC_INT_SHTDOWN_PEND)
	{
		_rsslSetError(error, NULL, RSSL_RET_FAILURE, errno);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT,
				"<%s:%d> rsslSocketGetSockOpts() failed due to session shutdown.\n",
				__FILE__, __LINE__);
...
void ipcUserSessClose(RsslSocketChannel *rsslSocketChannel)
{
	...
	*/
	...
	{
		if (rtrUnlikely(conndebug))
			printf("Cleanup old reconnect Stream FD: %p\n", rsslSocketChannel->oldTunnelStreamFd);
...
RsslInt32 ipcShutdownSession(RsslSocketChannel* rsslSocketChannel, RsslError *error)
{
	if (IPC_NULL_PTR(rsslSocketChannel, "ipcShutdownSession", "socket", error))
		return RSSL_RET_FAILURE;

#ifdef MUTEX_DEBUG
	printf("LOCK rsslSocketChannel -- ipcShutdownSession (start)\n");
#endif
	IPC_MUTEX_LOCK(rsslSocketChannel);

	/* clear up the socket struct */
	ipcUserSessClose(rsslSocketChannel);

#ifdef MUTEX_DEBUG
	printf("UNLOCK rsslSocketChannel -- ipcShutdownSession (end)\n");
#endif
	IPC_MUTEX_UNLOCK(rsslSocketChannel);
...
//corresponds to ipcShutdownSession(RIPC_SOCKET* socket, RsslError *error)
RsslInt32 ipcShutdownSockectChannel(RsslSocketChannel* rsslSocketChannel, RsslError *error)
{
	if (IPC_NULL_PTR(socket,"ipcShutdownSockectChannel","socket",error))
		return RSSL_RET_FAILURE;
...
ipcSessInit ipcSessionInit(RsslSocketChannel *rsslSocketChannel, ripcSessInProg *inPr, RsslError *error)
{
	...
	if (rtrUnlikely(conndebug))
		printf("ripcSessionInit() called\n");
	if (IPC_NULL_PTR(socket,"ipcSessionInit","socket",error))
		return(RIPC_CONN_ERROR);
...
RSSL_RSSL_SOCKET_IMPL_FAST(void) ripcClearRsslSocketChannel(RsslSocketChannel *rsslSocketChannel)
{
	...
	memset(rsslSocketChannel, 0, sizeof(RsslSocketChannel));
	...
	rsslSocketChannel->keyExchange = 0;
	//	rsslSocketChannel->transportFuncs = &(transFuncs[RIPC_SOCKET_TRANSPORT]);
	...
	//	rtr_qtool_headInit(&freeSessionList,rsslSocketChannel);
	//	rtr_qtool_init(&(rsslSocketChannel->userList), 0);
	...
	rtr_pipe_init(&rsslSocketChannel->sessPipe);
	...
	//	rsslSocketChannel->identifier = 0;
	...
	//	if (rsslSocketChannel->IPaddress)
	//		_rsslFree((void*)rsslSocketChannel->IPaddress);
	rsslSocketChannel->clientIP = 0;
	...
	//rsslSocketChannel->connectionType = RIPC_CONN_TYPE_INIT;
}
