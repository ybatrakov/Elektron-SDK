...
typedef struct
{
	...
	rtr_pipe			_bindPipe;		// not in shared memory segment
	rtr_pipe			_acceptPipe;	// not in shared memory segment. There is only once accept for shmem connection
...
typedef struct
{
	...
	rtr_pipe			_pipe;			// not stored in shared memory - used for signaling if needed
	...
	RsslUInt32			maxReaderSeqNumLag;		/* not stored in shared memory */
...
#define RSSL_SHMEM_INIT_CREATE_OPTS {0, 0, 0, 0, 0, 0, 0, 0, 0 }
...
#define RSSL_SHMEM_INIT_ATTACH_OPTS {0, 0, 0, 0, 0, 0, 0, 0 }
...
#if 1	// TODO: this code should be inlined, leave it here for now while I'm checking disassembly of 64 bit load/store
