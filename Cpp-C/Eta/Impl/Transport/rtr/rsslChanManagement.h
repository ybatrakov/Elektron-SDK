...
typedef struct {
	...
	rtr_qtool_head	link1;				/* storage for the activeChannelList qtool links */
	rtr_qtool_head	link2;				/* storage for the servers activeChannels qtool links */
	rtr_mutex		chanMutex;			/* internal mutex used for reads */
	rtr_mutex		traceMutex;			/* used when tracing to prevent multiple threads logging at the same time */
	rtr_qtool_list  activeBufferList;	/* list of active buffers by this channel */
	...
	rtr_hashtable 	assemblyBuffers;		/* list of assembly buffers */
	rtr_qtool_list	freeBufferList;			/* list of free buffers to use for writing */
...
typedef struct {
	...
	rtr_qtool_head	link1;					/* storage for the qtool links */
	rtr_mutex		srvrMutex;				/* internal mutex used for server */
	rtr_mutex		sharedBufPoolMutex;		/* mutex used for shared buf pool */
	...
	rtr_qtool_list	activeChannels;			/* list of channels connected to server */
...
typedef struct {
	rtr_qtool_head link1;				/* link used when this struct is put in a hashtable */
...
RTR_C_INLINE rtr_hash_sum UInt32_key_hash(void *element)
{
	RTPRECONDITION(key != 0);
	return ((*(rsslAssemblyBuffer*)element).fragId);
}

RTR_C_INLINE RTRBOOL checkFragID(void *element1, void *element2)
{
...
typedef struct RsslTransServerFuncs {
	...
	RsslRet(*closeServer)(rsslServerImpl* rsslChnlImpl, RsslError *error);
...
RTR_C_ALWAYS_INLINE rsslBufferImpl *_rsslCreateBuffer(rsslChannelImpl *chnl)
{
	...
	rtr_qtool_headInit(&(chnl->freeBufferList), buffer);
...
RTR_C_ALWAYS_INLINE rsslBufferImpl *_rsslNewBuffer(rsslChannelImpl *chnl)
{
	...
	if (multiThread == RSSL_LOCK_GLOBAL_AND_CHANNEL)
		RTR_MUTEX_LOCK(chnl->chanMutex, mterr);

	if ((buffer = (rsslBufferImpl*)rtr_qtool_first(&(chnl->freeBufferList))) == 0)
		buffer = _rsslCreateBuffer(chnl);
	else
	{
		if (rtr_qtool_headActive(&(chnl->freeBufferList), buffer))
		{
			rtr_qtool_remqe(&(chnl->freeBufferList), buffer);
			if (memoryDebug)
				printf("removing from freeBufferList\n");
		}
	}
	if (multiThread == RSSL_LOCK_GLOBAL_AND_CHANNEL)
		RTR_MUTEX_UNLOCK(chnl->chanMutex, mterr);
...
{
	chnl->Channel.socketId = -1;
	chnl->Channel.oldSocketId = -1;
	...
	/* activeBuffers list should be empty */

	rtr_hasht_init(&(chnl->assemblyBuffers), 0, 65535, checkFragID, UInt32_key_hash);
}
...
/* These are based off of ripc function (ripcGetServByName) 
