...
static unsigned char httpconndebug = 0;
...
RsslInt32 ipcGetHttpAck(char *buf, RsslInt32 length)
{
	...
	strLen = ripcHttpHdrToUpper(tmpBuf, length, 0);
...
RsslInt32 ipcGetHttpConnHeader(char *httpBuf, RsslInt32 length, RsslUInt16 headerSize, char *url, RsslUInt32 id1, RsslUInt32 id2)
{
	...
	if (url)
	{
		if ((RsslInt32)(strlen(url) + 150 + headerSize) > length)
			return 0;
		httpHeader = snprintf(httpBuf,length, "POST %s HTTP/1.0\r\n", url);
	} else
		httpHeader = snprintf(httpBuf,length, "POST / HTTP/1.0\r\n");
	
	httpHeader += snprintf(httpBuf + httpHeader,(length-httpHeader), "Pragma: no-cache\r\n");
	httpHeader += snprintf(httpBuf + httpHeader,(length-httpHeader), "Accept: image/gif, image/x-xbitmao, image/jpeg, image/pjpeg, image/png, */*\r\n");
	httpHeader += snprintf(httpBuf + httpHeader,(length-httpHeader), "Accept-Encoding: gzip\r\n");
	httpHeader += snprintf(httpBuf + httpHeader,(length-httpHeader), "Accept-Language: en\r\n");
	httpHeader += snprintf(httpBuf + httpHeader,(length-httpHeader), "Accept-Charset: iso-8859-1,*,utf-8\r\n");
	httpHeader += snprintf(httpBuf + httpHeader,(length-httpHeader), "Content-Length: %u\r\n", (headerSize+10));
	httpHeader += snprintf(httpBuf + httpHeader,(length-httpHeader), "Proxy-Connecion: Keep-Alive\r\n\r\n");
