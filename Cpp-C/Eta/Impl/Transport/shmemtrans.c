...
/* always do 8 byte alignment to ensure atomic reads/writes */
...
#else
	segSize = (rtrUInt32)	(RTR_SHM_ALIGNBYTES(sizeof(rtrUInt16) + sizeof(rtrUInt16)) +						/* shmemVersion, flags */
							 RTR_SHM_ALIGNBYTES(sizeof(rtrUInt32)) +											/* pingTimeout */
							 RTR_SHM_ALIGNBYTES(sizeof(rtrUInt32)) +											/* byteWritten */
							 RTR_SHM_ALIGNBYTES(sizeof(rtrUInt32)) +											/* protocolTyp */e
							 RTR_SHM_ALIGNBYTES(sizeof(rtrUInt32)) + 											/* majorVersion */ 
							 RTR_SHM_ALIGNBYTES(sizeof(rtrUInt32)) + 											/* minorVersion
							 RTR_SHM_ALIGNBYTES(sizeof(rtrUInt8) + RSSL_SHM_COMPONENT_VERSION_SIZE) +			/* component version and its length */
							 RTR_SHM_ALIGNBYTES(sizeof(rtrSpinLock)) +											/* userLock */
							 RTR_SHM_ALIGNBYTES(sizeof(rtrInt64)) +												/* seqNumServer */
							 RTR_SHM_ALIGNBYTES(sizeof(rtrShmCirBuf)));											/* circularBufferServer */
#endif
	...
	if (rtr_pipe_create(&trans->_bindPipe) < 1)
	{
		_rsslSetError(error, 0, RSSL_RET_FAILURE, 0);
	...
	if ((rtr_pipe_write(&trans->_bindPipe, "1", 1)) < 1)
	{
		_rsslSetError(error, 0, RSSL_RET_FAILURE, 0);
	...
	trans->_bindFD = rtr_pipe_get_read_fd(&trans->_bindPipe);
#endif
	...
	if (rtrShmSegCreate(&trans->shMemSeg,createOpts->shMemKey,segSize + (bufSize * createOpts->numBuffers), errBuff) < 0)
	{
		...
		rtr_pipe_close(&trans->_bindPipe);
		rtrReleaseMutex(trans->controlMutex);
		free(trans);
		return NULL;
	}
...
/* always do 8 byte alignment to ensure atomic reads/writes */
	...
	trans->seqNumServer = (rtr_atomic_val64*)rtrShmBytesReserve(&trans->shMemSeg, sizeof(rtrInt64));
	trans->circularBufferServer = (rtrShmCirBuf*)rtrShmBytesReserve( &trans->shMemSeg, sizeof(rtrShmCirBuf) );

#else
	/* 32 bit platforms are 4 byte aligned. we want to pack for optimal memory use */
	trans->shmemVersion = (rtrUInt16*)rtrShmBytesReserve( &trans->shMemSeg, (2 * sizeof(rtrUInt16)) );
	trans->flags = (rtrUInt16*)((char *)trans->shmemVersion + sizeof(rtrUInt16));
	trans->pingTimeout = (rtrUInt32*)rtrShmBytesReserve( &trans->shMemSeg, sizeof(rtrUInt32) );

	trans->byteWritten = (rtrUInt32*)rtrShmBytesReserve( &trans->shMemSeg, sizeof(rtrUInt32) );
	trans->protocolType = (rtrUInt32*)rtrShmBytesReserve( &trans->shMemSeg, sizeof(rtrUInt32) );

	trans->majorVersion = (rtrUInt32*)rtrShmBytesReserve( &trans->shMemSeg, sizeof(rtrUInt32) );
	trans->minorVersion = (rtrUInt32*)rtrShmBytesReserve( &trans->shMemSeg, sizeof(rtrUInt32) );

	trans->serverComponentVersionLen = (rtrUInt8 *)rtrShmBytesReserve(&trans->shMemSeg, RSSL_SHM_COMPONENT_VERSION_SIZE + sizeof(rtrUInt8));
	trans->serverComponentVersion = (char *)(trans->serverComponentVersionLen + sizeof(rtrUInt8));

	trans->userLock = (rtrSpinLock*)rtrShmBytesReserve( &trans->shMemSeg, sizeof(rtrSpinLock) );	/* 4 bytes on windows and linux */
	trans->seqNumServer = (rtr_atomic_val64*)rtrShmBytesReserve(&trans->shMemSeg, sizeof(rtrInt64));
	trans->circularBufferServer = (rtrShmCirBuf*)rtrShmBytesReserve( &trans->shMemSeg, sizeof(rtrShmCirBuf) );
#endif
...
int rtrShmTransAccept( rtrShmTransServer *trans, RsslError *error)
{
	...
	if (rtr_pipe_read(&trans->_bindPipe, tempBuf, 1) <= 0)
	{
		_rsslSetError(error, 0, RSSL_RET_FAILURE, 0);
	...
	if (rtr_pipe_create(&trans->_acceptPipe) < 1)
	{
		_rsslSetError(error, 0, RSSL_RET_FAILURE, 0);
	...
	trans->_acceptFD = rtr_pipe_get_read_fd(&trans->_acceptPipe);
#endif
...
int rtrShmTransDestroy( void *trans, RsslError *error )
	...
	*shmTransServerns->flags |= RSSL_SHM_SERVER_SHUTDOWN;	/* let the consumers know that the server is shutting down */
	...
	/* close the pipe(s) */
	rtr_pipe_close(&shmTransServerns->_bindPipe);
	rtr_pipe_close(&shmTransServerns->_acceptPipe);
	free(trans);
	return 0;
}
...
rtrShmTransClient* rtrShmTransAttach( rtrShmAttachOpts *attachOpts, RsslError *error )
{
	...
	if (rtr_pipe_create(&trans->_pipe) < 1)
	{
		_rsslSetError(error, 0, RSSL_RET_FAILURE, 0);
	...
	if (rtr_pipe_write(&trans->_pipe, "1", 1) < 1)
	{
		_rsslSetError(error, 0, RSSL_RET_FAILURE, 0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> rtrShmTransAttach unable to write byte to pipe.\n", __FILE__, __LINE__);
		rtr_pipe_close(&trans->_pipe);
		rtrReleaseMutex(trans->controlMutex);
		free(trans);
		return NULL;
	}
	trans->clientFD = rtr_pipe_get_read_fd(&trans->_pipe);
#endif
...
/* always do 8 byte alignment to ensure atomic reads/writes */
	...
	trans->seqNumServer = (rtr_atomic_val64*)rtrShmBytesAttach( &curLoc, sizeof(rtrInt64));
	trans->circularBufferServer = (rtrShmCirBuf*)rtrShmBytesAttach(&curLoc,sizeof(rtrShmCirBuf));

#else
	/* 32 bit platforms are 4 byte aligned. Pack for optimal memory use */
	trans->shmemVersion = (rtrUInt16*)rtrShmBytesAttach( &curLoc, sizeof(rtrInt32));
	trans->flags = (rtrUInt16*)((char *)trans->shmemVersion + sizeof(rtrUInt16));
	trans->pingTimeout = (rtrUInt32*)rtrShmBytesAttach( &curLoc, sizeof(rtrInt32));

	trans->byteWritten = (rtrUInt32*)rtrShmBytesAttach( &curLoc, sizeof(rtrInt32));
	trans->protocolType = (rtrUInt32*)rtrShmBytesAttach( &curLoc, sizeof(rtrInt32));

	trans->majorVersion = (rtrUInt32*)rtrShmBytesAttach( &curLoc, sizeof(rtrInt32));
	trans->minorVersion = (rtrUInt32*)rtrShmBytesAttach( &curLoc, sizeof(rtrInt32));

	trans->serverComponentVersionLen = (rtrUInt8 *)rtrShmBytesAttach(&curLoc, RSSL_SHM_COMPONENT_VERSION_SIZE + sizeof(rtrUInt8));
	trans->serverComponentVersion = (char *)(trans->serverComponentVersionLen + sizeof(rtrUInt8));

	trans->userLock = (rtrSpinLock*)rtrShmBytesAttach(&curLoc,sizeof(rtrSpinLock));
	trans->seqNumServer = (rtr_atomic_val64*)rtrShmBytesAttach( &curLoc, sizeof(rtrInt64));
	trans->circularBufferServer = (rtrShmCirBuf*)rtrShmBytesAttach(&curLoc,sizeof(rtrShmCirBuf));
#endif
	...
	{
		_rsslSetError(error, 0, RSSL_RET_FAILURE, 0);
		snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> maxReaderSeqNumLag(%d) greater than number of buffer(%d).\n", 
		...
		if (rtrLikely(reader_seqnum_lag != 0))
		{
			...
			*/
			...
			if (rtrUnlikely(reader_seqnum_lag > trans->maxReaderSeqNumLag))
			{
				...
				snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> rtrShmTransClientRead disconnected from shared memory because reader lags writer by %d messages.\n", __FILE__, __LINE__, reader_seqnum_lag);
	...
	else
	{
		...
		while (chnl->state == RSSL_CH_STATE_ACTIVE && trans->readBuffer == 0)
		{
			...
			if (rtrLikely(reader_seqnum_lag > 0))
			{
				...
				{
					_rsslSetError(error, 0, RSSL_RET_SLOW_READER, 0);
					snprintf(error->text, MAX_RSSL_ERROR_TEXT, "<%s:%d> rtrShmTransClientRead failed because reader lags writer by %d messages.\n", __FILE__, __LINE__, reader_seqnum_lag);
					chnl->state = RSSL_CH_STATE_CLOSED;
					return 0;
				}
				*readRet = (RsslRet) reader_seqnum_lag;
