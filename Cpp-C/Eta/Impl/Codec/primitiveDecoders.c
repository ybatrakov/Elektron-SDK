...
{
	((RsslBuffer*)value)->length = (pIter->_levelInfo[pIter->_decodingLevel + 1]._endBufPtr - pIter->_curBufPtr);
...
RsslRet RTR_FASTCALL _rsslDecInt(RsslDecodeIterator *pIter, void *value)
{
	return( ((pIter->_levelInfo[pIter->_decodingLevel + 1]._endBufPtr - pIter->_curBufPtr) == 0) ?
			RSSL_RET_BLANK_DATA :
		 (rwfGetLenSpecI64Size((RsslInt64*)value, pIter->_curBufPtr, (pIter->_levelInfo[pIter->_decodingLevel + 1]._endBufPtr - pIter->_curBufPtr)) > -1) ?
		  RSSL_RET_SUCCESS :
...
RsslRet RTR_FASTCALL _rsslDecUInt(RsslDecodeIterator *pIter, void *value)
{
	return( ((pIter->_levelInfo[pIter->_decodingLevel + 1]._endBufPtr - pIter->_curBufPtr) == 0) ?
			RSSL_RET_BLANK_DATA :
		(rwfGetLenSpecU64Size((RsslUInt64*)value, pIter->_curBufPtr, (pIter->_levelInfo[pIter->_decodingLevel + 1]._endBufPtr - pIter->_curBufPtr)) > -1) ?
		  RSSL_RET_SUCCESS :
		RSSL_RET_INCOMPLETE_DATA );
}
...
RsslRet RTR_FASTCALL _rsslDecReal(RsslDecodeIterator *pIter, void *value)
{
	...
	switch ((pIter->_levelInfo[pIter->_decodingLevel + 1]._endBufPtr - pIter->_curBufPtr))
	{
		...
		break;
		default:
			if (rwfGetReal64(&pReal->value, &format, (pIter->_levelInfo[pIter->_decodingLevel + 1]._endBufPtr - pIter->_curBufPtr), pIter->_curBufPtr ) > 0)
			{
				switch(format & 0x3F)
				{
					case RSSL_RH_INFINITY:
...
RsslRet RTR_FASTCALL _rsslDecEnum(RsslDecodeIterator *pIter, void *value)
{
	return( ((pIter->_levelInfo[pIter->_decodingLevel + 1]._endBufPtr - pIter->_curBufPtr) == 0) ?
		RSSL_RET_BLANK_DATA :
		(rwfGetLenSpecU16_Size(value, pIter->_curBufPtr, (pIter->_levelInfo[pIter->_decodingLevel + 1]._endBufPtr - pIter->_curBufPtr)) > -1) ?
	   RSSL_RET_SUCCESS :
	   RSSL_RET_INCOMPLETE_DATA );
}
...
{	
	...
	if ((ret = rsslDecodeDate(pIter, &dateTime.date)) < 0) /* Decode value */
		return ret;
	if (ret == RSSL_RET_BLANK_DATA)
		return _blankAsString(oBuffer); /* If blank, clear string */
	if ((ret2 = rsslDateTimeToString(oBuffer, RSSL_DT_DATE, &dateTime)) < 0)
		return ret2; /* Else write string */
	return ret; /* Return code from decode function(will usually just be success) */
}
...
RsslRet RTR_FASTCALL _rsslDateToString(void *pDate, RsslBuffer *oBuffer)
{
	...
	return rsslDateTimeToString(oBuffer, RSSL_DT_DATE, &dateTime);
}
...
RsslRet RTR_FASTCALL _rsslTimeAsString(RsslDecodeIterator *pIter, RsslBuffer *oBuffer)
{
	...
	if ((ret = rsslDecodeTime(pIter, &dateTime.time)) < 0) /* Decode value */
		return ret;
	if (ret == RSSL_RET_BLANK_DATA)
		return _blankAsString(oBuffer); /* If blank, clear string */
	if ((ret2 = rsslDateTimeToString(oBuffer, RSSL_DT_TIME, &dateTime)) < 0)
		return ret2; /* Else write string */
	return ret; /* Return code from decode function(will usually just be success) */
}
...
RsslRet RTR_FASTCALL _rsslTimeToString(void *pTime, RsslBuffer *oBuffer)
{
	...
	return rsslDateTimeToString(oBuffer, RSSL_DT_TIME, &dateTime);
}
...
RsslRet RTR_FASTCALL _rsslDateTimeAsString(RsslDecodeIterator *pIter, RsslBuffer *oBuffer)
{
	...
	if ((ret = rsslDecodeDateTime(pIter, &dateTime)) < 0) /* Decode value */
		return ret;
	if (ret == RSSL_RET_BLANK_DATA)
		return _blankAsString(oBuffer); /* If blank, clear string */
	if ((ret2 = rsslDateTimeToString(oBuffer, RSSL_DT_DATETIME, &dateTime)) < 0)
		return ret2; /* Else write string */
	return ret; /* Return code from decode function(will usually just be success) */
}
...
{
	return rsslDateTimeToString(oBuffer, RSSL_DT_DATETIME, (RsslDateTime*)pDateTime);
}
...
{
	RsslBuffer iBuffer = { (pIter->_levelInfo[pIter->_decodingLevel + 1]._endBufPtr - pIter->_curBufPtr), pIter->_curBufPtr };
...
RSSL_API RsslRet rsslDecodeReal(RsslDecodeIterator *pIter, RsslReal *value)
{
	...
	switch ((pIter->_levelInfo[pIter->_decodingLevel + 1]._endBufPtr - pIter->_curBufPtr))
	{
		...
		break;
		default:
			if (rwfGetReal64(&value->value, &format, (pIter->_levelInfo[pIter->_decodingLevel + 1]._endBufPtr - pIter->_curBufPtr), pIter->_curBufPtr ) > 0)
			{
				switch(format & 0x3F)
				{
					case RSSL_RH_INFINITY:
...
RSSL_API RsslRet rsslDecodeBuffer(RsslDecodeIterator *pIter, RsslBuffer *value)
{
	...
	value->length = pIter->_levelInfo[pIter->_decodingLevel + 1]._endBufPtr - pIter->_curBufPtr;
