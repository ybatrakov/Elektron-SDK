...
/**
 ** @brief Checks for valid pointer within a buffer.
 ** @see RsslBuffer
 **/
//#define _rsslValidBufferPointer( curPtr, bufPtr ) \
//	(curPtr <= ((bufPtr)->data + (bufPtr)->length))
//
/**
 ** @brief Checks for invalid pointer within a buffer.
 ** @see RsslBuffer
 **/
//#define _rsslInvalidBufferPointer( curPtr, bufPtr ) \
//	(curPtr > ((bufPtr)->data + (bufPtr)->length))
//
/**
 ** @brief Checks for valid pointer within a buffer given the end pointer.
 ** @see RsslBuffer
 **/
//#define _rsslValidBufferPointerEndPtr( curPtr, endPtr ) (curPtr <= endPtr)
//
/**
 ** @brief Checks for invalid pointer within a buffer given the end pointer.
 ** @see RsslBuffer
 **/
//#define _rsslInvalidBufferPointerEndPtr( curPtr, endPtr ) (curPtr > endPtr)
//
/**
 ** @brief Checks for buffer overrun if length bytes appended.
 ** @see RsslBuffer
 **/
//#define _rsslBufferOverrun( curPtr, length, bufPtr ) \
//	((curPtr + length) > ((bufPtr)->data + (bufPtr)->length))
...
//#if 0
//typedef struct
//	rsslDAFValSingleFld	= 0x0010,	/* Valid For Single Field */
//	rsslDAFValAggFld	= 0x0020,	/* Valid For Aggregate Field */
//	rsslDAFValSingleEl	= 0x0040,	/* Valid For Single Element */
//	rsslDAFValAggEl		= 0x0080,	/* Valid For Aggregate Element */
//	rsslDAFValMapKey	= 0x0100	/* Valid For Aggregate Element */
//} RsslDataAssertionFlags;
//#endif
...
//ETAC_OPEN_SOURCE_DOUBLE_CHECK
//#define _rsslEncodeItem( PITER, DT, PDATA ) \
//	( (DT < RSSL_DT_LAST) && (_rsslDataTypeInfo[DT].rawEncoders != 0) ?  \
//			(*(_rsslDataTypeInfo[DT].rawEncoders))(PITER,PDATA) : \
//			RSSL_RET_UNSUPPORTED_DATA_TYPE )
//
//#define _rsslDecodePrimitive( PITER, DT, PBUFFER ) \
//	( (DT < RSSL_DT_SET_PRIMITIVE_MIN) && (_rsslDataTypeInfo[DT].rawDecoders != 0) ?  \
...
//ETAC_OPEN_SOURCE_DOUBLE_CHECK
//RTR_C_ALWAYS_INLINE RsslBool _rsslValidDataType( RsslDataType type )
//{
//	return((type < RSSL_DT_LAST) ? (_rsslDataTypeInfo[type].flags != 0) : 0);
//}
...
//ETAC_OPEN_SOURCE_DOUBLE_CHECK
//RTR_C_ALWAYS_INLINE RsslBool _rsslValidSetDataType( RsslDataType type )
//{
//	return((type < RSSL_DT_CONTAINER_TYPE_MIN) ? (_rsslDataTypeInfo[type].flags != 0) : 0);
//}
...
//ETAC_OPEN_SOURCE_DOUBLE_CHECK
//RTR_C_ALWAYS_INLINE int _rsslDataTypeLenSpecEnc(RsslDataType type )
//{
//	return((type < RSSL_DT_LAST) ?
//			(_rsslDataTypeInfo[type].flags & RsslDTFLenSpecEnc) : 0);
//}
//
//RTR_C_ALWAYS_INLINE int _rsslDataTypeFixLenEnc(RsslDataType type )
//{
//	return((type < RSSL_DT_LAST) ?
//			(_rsslDataTypeInfo[type].flags & RsslDTFFixLenEnc) : 0);
//}
//
//RTR_C_ALWAYS_INLINE int _rsslDataTypeVarLenEnc(RsslDataType type )
//{
//	return((type < RSSL_DT_LAST) ?
//			(_rsslDataTypeInfo[type].flags & RsslDTFVarLenEnc) : 0);
//}


//ETAC_OPEN_SOURCE_DOUBLE_CHECK
///*
// * Changes vector data format on the wire - this is for internal use only 
// * DO NOT USE IF YOU DO NOT UNDERSTAND
// */
//RTR_C_ALWAYS_INLINE void rsslVectorChangeContainerType(RsslEncodeIterator *pVectorIter, RsslContainerType containerType)
//{
//	RsslEncodingLevel *levelInfo = &pVectorIter->_levelInfo[pVectorIter->_encodingLevel];
//	RsslContainerType tempCt = containerType;
//	RSSL_ASSERT(
//		(tempCt >= RSSL_DT_CONTAINER_TYPE_MIN && tempCt <= RSSL_DT_CONTAINER_TYPE_MAX) ||
//		(tempCt >= RSSL_DT_MAX_RESERVED && tempCt <= RSSL_DT_LAST), 
//		Invalid type);
//	/* needs to be scaled before encoding */
//	tempCt -= RSSL_DT_CONTAINER_TYPE_MIN;
//	RWF_MOVE_8(&levelInfo->_containerStartPos[1], &tempCt);
//	((RsslVector*)levelInfo->_listType)->containerType = containerType;
//}
//
///*
// * Changes map container type on the wire - this is for internal use only
// * DO NOT USE IF YOU DO NOT UNDERSTAND 
// */
//RTR_C_ALWAYS_INLINE void rsslMapChangeContainerType(RsslEncodeIterator *pMapIter, RsslContainerType containerType)
//{
//	RsslEncodingLevel *levelInfo = &pMapIter->_levelInfo[pMapIter->_encodingLevel];
//      RsslContainerType tempCt = containerType;
//	RSSL_ASSERT(
//		(tempCt >= RSSL_DT_CONTAINER_TYPE_MIN && tempCt <= RSSL_DT_CONTAINER_TYPE_MAX) ||
//		(tempCt >= RSSL_DT_MAX_RESERVED && tempCt <= RSSL_DT_LAST), 
//		Invalid type);
//      /* need to scale container type before encoding */
//      tempCt -= RSSL_DT_CONTAINER_TYPE_MIN;
//      RWF_MOVE_8(&levelInfo->_containerStartPos[2], &tempCt);
//      ((RsslMap*)levelInfo->_listType)->containerType = containerType;
//}
...
//ETAC_OPEN_SOURCE_DOUBLE_CHECK
//#define __RSZMTM    sizeof(RsslUInt32)
...
//ETAC_OPEN_SOURCE_DOUBLE_CHECK
//#define __RSZEX32   sizeof(RsslUInt8) + sizeof(RsslUInt32)
//#define __RSZEX64   sizeof(RsslUInt8) + sizeof(RsslUInt64)
