...
int readMsg(FILE * file, RsslUInt8 *majorVer, RsslUInt8 *minorVer, RsslBuffer * buffer)
{
	...
	ret = fread( data, sizeof( RsslUInt32), 1, file );
	if (ferror(file) || (ret == 0)) return -1;

	RWF_MOVE_32(&msgSize, data);

	/* read major version */
	ret = fread( majorVer, sizeof( RsslUInt8), 1, file );
	if (ferror(file) || (ret == 0)) return -1;
	
	/* read major version */
	ret = fread( minorVer, sizeof( RsslUInt8), 1, file );
	if (ferror(file) || (ret == 0)) return -1;
	...
	ret = fread( buffer->data, 1, msgSize, file );
	bytesRead = ret;
...
int writeMsg(FILE * out, RsslBuffer * buffer, RsslUInt8 majorVer, RsslUInt8 minorVer, RsslUInt16 len)
{
	...
	ret = fwrite(&count, 2, 1, out);
	ret = fwrite(&majorVer, 1, 1, out);
	ret = fwrite(&minorVer, 1, 1, out);
	ret = fwrite(buffer->data, 1, len, out);
	return ret;
}
