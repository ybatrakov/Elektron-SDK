...
const char* getInstrumentNameTypeAsString (RDMInstrumentNameTypes symbol)
{
	switch (symbol)
	{
...
int decodeElementListToXML(FILE * file, RsslDecodeIterator *iter, const RsslDataDictionary * dictionary, RsslLocalElementSetDefDb * setDb)
{
	...
	while ((ret = rsslDecodeElementEntry(iter, &element)) != RSSL_RET_END_OF_CONTAINER)
	{
		...
		if ((element.dataType >= RSSL_DT_CONTAINER_TYPE_MIN && element.dataType <= RSSL_DT_CONTAINER_TYPE_MAX) || 
			...
			(element.dataType >= RSSL_DT_MAX_RESERVED && element.dataType <= RSSL_DT_LAST))
			fprintf(file, ">\n");
...
int decodeFieldListToXML(FILE * file, RsslDecodeIterator *iter, const RsslDataDictionary * dictionary, RsslLocalFieldSetDefDb * fieldListSetDb)
{
	int i = 0;
	RsslRet ret = 0;
	...
	while ((ret = rsslDecodeFieldEntry(iter, &field)) != RSSL_RET_END_OF_CONTAINER)
	{
		...
		if ((dataType >= RSSL_DT_CONTAINER_TYPE_MIN && dataType <= RSSL_DT_CONTAINER_TYPE_MAX) || 
			...
			(dataType >= RSSL_DT_MAX_RESERVED && dataType <= RSSL_DT_LAST))
			fprintf( file, ">\n");
...
int decodeSummaryData(FILE *file, RsslDecodeIterator *iter, RsslContainerType containerType, const RsslBuffer * input, const RsslUInt8 majorVer, const RsslUInt8 minorVer, const RsslDataDictionary * dictionary, void * setDb)
{
	int ret = 0;
...
RsslRet decodeKeyOpaque(FILE * file,
