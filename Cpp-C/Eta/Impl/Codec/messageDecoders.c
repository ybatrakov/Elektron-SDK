...
RSSL_API RsslRet rsslDecodeMsg(RsslDecodeIterator * dIter, RsslMsg * msg)
{
	...
	msg->msgBase.encDataBody.length = (_levelInfo->_endBufPtr - position);
...
RSSL_API RsslRet rsslDecodeMsgHeader(const RsslDecodeIterator * dIter, RsslMsg * msg)
{
	...
	msg->msgBase.encMsgBuffer.length = dIter->_levelInfo[dIter->_decodingLevel]._endBufPtr - dIter->_curBufPtr;
	...
	case RSSL_MC_UPDATE:
		...
		if (msg->updateMsg.flags & RSSL_UPMF_HAS_POST_USER_INFO)
		{
			...
			if ((headerSize - (position - hdrStartPos)) >= sizeof(msg->updateMsg.postUserInfo))
			{
				/* get user address */
	...
	case RSSL_MC_REFRESH:
		...
		{
			/* decode only if actually in the header */
			if ((headerSize - (position - hdrStartPos)) >= sizeof(msg->refreshMsg.postUserInfo))
			{
				/* get user address */
	...
	case RSSL_MC_STATUS:
		...
		{
			/* decode only if actually in the header */
			if ((headerSize - (position - hdrStartPos)) >= sizeof(msg->statusMsg.postUserInfo))
			{
				/* get user address */
	...
	return position - dIter->_curBufPtr;
}
...
RsslRet rsslExtractDataSection(RsslBuffer * data, const RsslMsg * msg)
{
	...
	data->data = start;
	if (start)
		data->length = msg->msgBase.encDataBody.data + msg->msgBase.encDataBody.length - start;
	else
		data->length = 0;
	return RSSL_RET_SUCCESS;
}
