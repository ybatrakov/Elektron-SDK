	...
	*pReqKeyAttrib = RSSL_FALSE;
	...
	if (key)
	{
		...
		if (!(*pReqKeyAttrib))
		{
			...
			keySize = (pIter->_curBufPtr - lenPos - 2);
			/* now set the RB bit */
			keySize |= 0x8000;
								...
								RsslMsg					*pMsg )
{
	...
	/* write header size */
	headerSize = pIter->_curBufPtr - _levelInfo->_countWritePtr - 2;
													...
													RsslBool			success )
{
	...
	{	
		if (_levelInfo->_internalMark._sizeBytes > 0)
		{
			...
			size = (pIter->_curBufPtr - _levelInfo->_internalMark._sizePtr - _levelInfo->_internalMark._sizeBytes);
