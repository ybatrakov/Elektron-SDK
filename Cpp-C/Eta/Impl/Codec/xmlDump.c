...
void xmlDumpReal(FILE *file, RsslReal * value)
{
	...
	convValue.hint = value->hint;
	convValue.isBlank = value->isBlank;
	convValue.value = value->value;
...
void xmlDumpMapEntryBegin(FILE *file, RsslDataType keyPrimitiveType, RsslMapEntry *mEntry, void * pMapKeyData)
{
	...
	RsslDecodeIterator iter = RSSL_INIT_DECODE_ITERATOR;
	char data[1024];
...
void xmlDumpSeriesBegin(FILE *file, RsslSeries *ser)
{
	RsslBool firstFlag = RSSL_TRUE;
...
void xmlDumpSeriesRowBegin(FILE *file, RsslSeriesEntry *sRow)
{
	encodeindents(file);
	indents++;
...
void xmlDumpTimestamp(FILE *file)
{
	...
	struct _timeb	_time;
	_ftime(&_time);
	sec = _time.time - 60 * (_time.timezone - _time.dstflag * 60);
	min = sec / 60 % 60;
	hour = sec / 3600 % 24;
	sec = sec % 60;
	msec = _time.millitm;
	...
	fprintf(file, "<!-- Time: %d:%02d:%02d:%03d -->\n", 
		hour,
		min,
		sec,
		msec);
